...
\includegraphics[width=\textwidth]{../supplied_graphics/example_exp.png}

}

\caption[Experimental trial example from Experiments 1 to
4.]{\label{fig-exp-illustrate}An example of an experimental trial in
Experiments 1-4, including the scatterplot stimulus (A), the slider that
participants were instructed to use to estimate correlation (B), and the
trial counter (C).}
...
experimental items. In Experiments 1 to 4, each participant saw all
experimental stimuli and provided a judgement of correlation using a
sliding scale between 0 and 1 (see the example of an experimental trial
in Figure~\ref{fig-exp-illustrate}). Experiment 1 featured a single
factor of global scatterplot point opacity with 4 levels (see
Figure~\ref{fig-exp1-examples}). Experiment 2 featured a single factor
of scatterplot point design regarding opacity with 4 levels (see
Figure~\ref{fig-exp2-examples}). Experiment 3 featured a single factor
of scatterplot point design regarding size with 4 levels (see
Figure~\ref{fig-exp3-examples}). Experiment 4 featured a factorial
\(2\times2\) design; IV\textsubscript{1} was the scatterplot point
...
1-factor, 2-level between-participants design; group A saw alternative
scatterplots designed to elicit greater levels of belief change compared
to standard scatterplots, which were shown to group B (see
Figure~\ref{fig-exp5-examples}). Characteristics of all experiments are
summarised in Table~\ref{tbl-exp-summary}, which also includes locations
for full descriptions of stimuli.
...
1.]{\label{fig-exp1-examples}Examples of the stimuli used in Experiment
1, demonstrated with \textit{r} values of 0.2, 0.6, and 0.99. Here,
...
\caption[Examples of the stimuli used in Experiment
2.]{\label{fig-exp2-examples}Examples of the stimuli used in Experiment
2, demonstrated with \textit{r} values of 0.2, 0.6, and 0.99.}
...
\caption[Examples of the stimuli used in Experiment
3.]{\label{fig-exp3-examples}Examples of the stimuli used in Experiment
3, demonstrated with \textit{r} values of 0.2, 0.6, and 0.99.}
...
\caption[Examples of the stimuli used in Experiment
4.]{\label{fig-exp4-examples}Examples of the stimuli used in Experiment
4, demonstrated with \textit{r} values of 0.2, 0.6, and 0.99.}
...
\caption[Examples of the stimuli used in Experiment
5.]{\label{fig-exp5-examples}Examples of the stimuli for Experiment 5
using \textit{r} values of 0.2, 0.6, and 0.99. Group A saw the
alternative scatterplot presented on the left, while group B saw the
standard design on the right.}
...
\includegraphics[width=\textwidth]{3_general_methodology_files/figure-latex/fig-exp5-examples-cont-1.pdf}

}

\caption{\label{fig-exp5-examples-cont}(Continued)}

\end{figure}%

\begin{table}

\caption{\label{tbl-exp-summary}Experimental summary table showing the
visual channel manipulated, the type of manipulation, dependent
variable, presence or absence of a baseline condition, experimental
design, section, and chapter.}

\centering{

\centering
\resizebox{\ifdim\width>\linewidth\linewidth\else\width\fi}{!}{
\begin{tabular}[t]{>{\raggedleft\arraybackslash}p{1cm}>{\raggedright\arraybackslash}p{2cm}>{\raggedright\arraybackslash}p{2.5cm}>{\raggedright\arraybackslash}p{2.5cm}>{\raggedright\arraybackslash}p{2cm}>{\raggedright\arraybackslash}p{2.5cm}>{\raggedright\arraybackslash}p{1.5cm}>{\raggedright\arraybackslash}p{1.5cm}}
\toprule
Exp. & Visual Channel & Manipulation Type & Dependent Variable & Baseline Condition & Design & Section & Chapter\\
\midrule
1 & Opacity & Global & \textit{r} estimate & Yes & Within-Subjects & \ref{design-e1} & \chap{chap:adjusting_opacity}\\
2 & Opacity & Spatially Dependent & \textit{r} estimate & Yes & Within-Subjects & \ref{design-e2} & \chap{chap:adjusting_opacity}\\
3 & Size & Spatially Dependent & \textit{r} estimate & Yes & Within-Subjects & \ref{stimuli-e3} & \chap{chap:adjusting_size}\\
4 & Opacity + Size & Spatially Dependent & \textit{r} estimate & No & Within-Subjects & \ref{stimuli-e4} & \chap{chap:interactions_opacity_size}\\
5 & Opacity + Size & Spatially Dependent & Belief change & No & Between-Subjects & \ref{stimuli-e5} & \chap{chap:belief_change}\\
\bottomrule
\end{tabular}}
...
\subsection{A Note on Ink}\label{ink}

Overlap between points was not taken into account during any of the
experiments described in this thesis. As \emph{r} increases and points
become closer together, the amount of overlap also increases. Due to
this, the total proportion of the plot occupied decreases. I refer to
this concept as the total amount of \emph{ink} present in a particular
plot. Below, I present visualisations of the amount of ink present in a
scatterplot, and how this changes as a function of the \emph{r} value
the plot describes. These visualisations take into account point opacity
and point overlap. The ink distribution plot for experiment 1 can be
seen in Figure~\ref{fig-ink-e1}, for experiment 2 in
Figure~\ref{fig-ink-e2}, for experiment 3 in Figure~\ref{fig-ink-e3},
for experiment 4 in Figure~\ref{fig-ink-e4}, and for experiment 5 in
Figure~\ref{fig-ink-e5}. In all cases, total ink values only refer to
the plotted scatterplot points in relation to the white plotting
background; all other visual features, including axis lines and ticks in
experiments 1 to 4, and text in experiment 5, are discounted. Each
visualisation shows the normalised total ink
(\(\frac{total\ ink}{total\ pixel\ count}\)) on the \(y\)-axis.

\begin{figure}

\centering{

\includegraphics[width=\textwidth]{3_general_methodology_files/figure-latex/fig-ink-e1-1.pdf}

}

\caption[Total ink plot for Experiment 1.]{\label{fig-ink-e1}Changes in
the total amount of ink in plots in experiment 1 as a function of
\textit{r} value, plotted separately for the four global opacity
conditions. Note how in each condition, the total amount of ink begins
to drop very slightly as the objective \textit{r} value increases.}
...
\includegraphics[width=\textwidth]{3_general_methodology_files/figure-latex/fig-ink-e2-1.pdf}

}

\caption[Total ink plot for Experiment 2.]{\label{fig-ink-e2}Changes in
the total amount of ink in plots in experiment 2 as a function of
\textit{r} value, plotted separately for the three opacity decay
conditions and the full opacity condition.}
...
\includegraphics[width=\textwidth]{3_general_methodology_files/figure-latex/fig-ink-e3-1.pdf}

}

\caption[Total ink plot for Experiment 3.]{\label{fig-ink-e3}Changes in
the total amount of ink in plots in experiment 3 as a function of
\textit{r} value, plotted separately for the three size decay conditions
and the standard size condition. The influence of point overlap causes
the total amount of ink to fall in all conditions, with non-linear decay
experiencing some rise in total ink prior.}
...
\includegraphics[width=\textwidth]{3_general_methodology_files/figure-latex/fig-ink-e4-1.pdf}

}

\caption[Total ink plot for Experiment 4.]{\label{fig-ink-e4}Changes in
the total amount of ink in plots in experiment 4 as a function of
\textit{r} value, plotted separately for the four opacity and size decay
conditions.}
...
\includegraphics[width=\textwidth]{3_general_methodology_files/figure-latex/fig-ink-e5-1.pdf}

}

\caption[Total ink plot for Experiment 5.]{\label{fig-ink-e5}Changes in
the total amount of ink in plots in experiment 5 as a function of
\textit{r} value, plotted separately for standard and alternative
plotting conditions. Only the scatterplot points themselves are included
...
high level of timings accuracy \cite{bridges_2020}. Using such an
...
my familiarity with the system. Despite these findings, there has been
evidence of low data quality and skewed demographics affecting both
...
\cite{charalambides_2021}, leading to a participant pool heavily skewed
towards young people identifying as female. At the time, Prolific did
not manually balance the participants recruited for a study. This was
addressed in the pilot study (see Section \ref{pilot-study} on page
\pageref{pilot-study}) by preventing participants who joined after this
date from participating, in addition to manually requesting a 1:1 ratio
of male to female participants. The demographic issues settled quickly,
however the screened 1:1 ratio was maintained for the remainder of the
experiments. Participants were only permitted to complete any of the
experiments on desktop or laptop computers, not on phones or tablets.

Chronologically, the first experiment conducted was a pilot study (see
Section \ref{pilot-study} on page \pageref{pilot-study} for full
details) investigating a very early iteration of the point opacity
manipulation in combination with exploratory work around plot size and
correlation estimation. At the time, I was relatively naive to the
intricacies of recruiting research participants online, and thus
experienced issues regarding participant engagement. Each experiment
included attention check questions in which participants were instructed
to ignore the stimulus and provide a specific answer. The advert for
each experiment stated that failure of more than 2 attention check items
would result in a submission being rejected. This pilot study suffered
from a rejection rate of 57.5\%, indicating very low levels of
participant engagement. For the following studies, published guidelines
\cite{peer_2021} were followed to address these issues; specifically, it
was required that participants:
...
rate for the next experiment fell to \sim 5\%. Rejection rates were
similar for the remainder of experiments. Exact numbers of accepted and
rejected participants can be found in the \textbf{Participants} sections
of each experiment.
...
size (Cohen's \emph{d} \sim 0.2), and aimed to recruit enough
participants to adequately power the experiments \cite{brysbaert_2019}.
NB: I did not conduct an \emph{a priori} power analysis. A post-hoc
power analysis of the first experiment revealed a power of 0.54. Effect
sizes were larger in the subsequent experiments, however to facilitate
comparison, it was decided that \emph{N} = 150 would remain the target
recruitment rate.
...
and titles. The axis ticks themselves were preserved. Scatterplots
always featured 128 data points and points had a hard size minimum of 12
pixels. Figure~\ref{fig-scatter-example} demonstrates the basic design
of the scatterplots used in Experiments 1 to 4.
...
All analyses in this thesis were conducted using R (version 4.5.3
...
significant differences in the means of the dependent variables between
...
\caption[Visualising random intercepts and slopes for mixed effects
models.]{\label{fig-mixed-effects-demo}Visualising random intercepts and
...
variability present in the datasets. In order to ascertain the
...
experiments bar Experiment 5), the \texttt{emmeans} package (version
1.11.2-8 \cite{lenth_2024}) is used to calculate estimated marginal
...
is therefore considered inappropriate for modelling Likert scale
response data \cite{liddell_2018}. In light of these issues, the
\texttt{ordinal} package (version 2023.12-4.1 \cite{ordinal}) in R was
used to build cumulative link mixed-effects models for the analysis of
Likert scale data. This allows for the treatment of Likert responses as
...
\texttt{buildmer} package (version 2.12 \cite{buildmer}) in R was used
to automate the selection of model specifications. Having been provided
with a maximal model, \texttt{buildmer} uses stepwise regression to
select the most complex model structure that successfully converges.
Following this, terms that fail to explain a significant amount of
...
deemed inappropriate. The \texttt{r2glmm} package (version 0.1.3
\cite{r2glmm}) was used instead to calculate semi-partial
R\textsuperscript{2}. In lieu of a traditional measure of effect size,
this demonstrated the unique variance in the dependent variable
explained by each level of the independent \cite{nakagawa_2013}.
Experiment 5 features a much simpler modelling situation, and returns to
providing equivalent Cohen's \emph{d} values for the pre- vs.~post- plot
viewing conditions, this time calculated by converting odds ratios using
the \texttt{effectsize} package (version 1.0.1 \cite{effectsize}). More
details on specific calculations, measures, and conclusions can be found
\emph{in situ}.
...
what would otherwise be a makeshift script followed by copy-pasting
...
application's file access to a `chroot jail'. Since then,
containerisation software has experienced rapid development and uptake,
mostly within the software development and security communities. Docker,
released in 2013, is a popular, lightweight containerisation tool that
...
analysis. The \texttt{renv} package (version 1.1.8 \cite{renv}) is used
to capture package versions and dependencies.

Below is a minimal example of a Dockerfile.

I first specify the Rocker image that will form the basis of the
container. This includes the version of R required (version 4.5.3), the
Rstudio Integrated Development Environment (IDE), Quarto, and the
\texttt{tidyverse} package.

\texttt{FROM\ rocker/version:4.5.3}

Then, I add the required \texttt{renv} files that I have previously used
to capture packages, versions, and dependencies:
...
Following that, I instruct \texttt{renv} to use the \texttt{renv.lock}
file it generated to install the exact versions of the packages the
project requires:

\texttt{RUN\ R\ -e\ "renv::restore()"}

Next, I add the files and folders required, including the Quarto setup
file, chapter folder, bibliography, additional scripts, LaTeX class file
and template, the folders containing the raw data and cached models, and
the R project file:
...
Finally, I use \texttt{tinytex} to install necessary additional LaTeX
files that are required to produce the final document. For brevity, I
only show the first few here:
...
ranging from poor practice (e.g.~Potti et al., 2006) to outright
deception and fabrication (e.g.~the Woo-Suk Hwang scandal
...
\caption[Peng's (2011) Reproducibility
Spectrum.]{\label{fig-reproducibility-spectrum}Peng's (2011)
...
implementation of Docker is provided to reproduce this thesis. All
...
\href{https://gitlab.pavlovia.org/Strain/exp_uniform_adjustments}{gitlab.pavlovia.org/Strain/exp\_uniform\_adjustments}

Experiment 2:

\href{https://gitlab.pavlovia.org/Strain/exp_spatially_dependent}{gitlab.pavlovia.org/Strain/exp\_spatially\_dependent}

\textbf{Chapter 5}

Experiment 3:

\href{https://gitlab.pavlovia.org/Strain/exp_size_only}{gitlab.pavlovia.org/Strain/exp\_size\_only}

\textbf{Chapter 6}

Experiment 4:

\href{https://gitlab.pavlovia.org/Strain/size_and_opacity_additive_exp}{gitlab.pavlovia.org/Strain/size\_and\_opacity\_additive\_exp}

\textbf{Chapter 7}

Experiment 5 Pre-Test:

\href{https://gitlab.pavlovia.org/Strain/beliefs_scatterplots_pretest}{gitlab.pavlovia.org/Strain/beliefs\_scatterplots\_pretest}

Experiment 5 Main Study (Group A):

\href{https://gitlab.pavlovia.org/Strain/atypical_scatterplots_main_a}{gitlab.pavlovia.org/Strain/atypical\_scatterplots\_main\_a}

Experiment 5 Main Study (Group B):

\href{https://gitlab.pavlovia.org/Strain/atypical_scatterplots_main_t}{gitlab.pavlovia.org/Strain/atypical\_scatterplots\_main\_t}
...
have produced a template\footnote{https://github.com/gjpstrain/UoM\_reproducible\_thesis\_template}
