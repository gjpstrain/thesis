...
\caption[Early time series visualisation from Funkhouser (1936)
\cite{funkhouser_1936}.]{\label{fig-early-time-series}Reproduced in
Tufte and Graves-Morris, 1983 \cite{tufte_1983} from Funkhouser, 1936
...
here in Figure~\ref{fig-early-time-series}, and purports to show the
movements of planetary bodies as a function of time, although Funkhouser
considered it little more than a ``schematic diagram\ldots for
illustrative purposes'' \cite{funkhouser_1936}. Regardless, the
recognisable grid lines and sinusoidal variation in the curves are ideas
that would not appear again for another 600-700 years, after which they
would become mainstream visualisation techniques. In the mid-14th
century, French philosopher Nicole Oresme demonstrated an understanding
of graphing by plotting proto-bar charts, and by the 16th century,
advances in cartography, photography, and mathematics laid the ground
for an explosion in data visualisation.
...
\caption[Scheiner's (1626) sunspot plot.]{\label{fig-sunspots}Scheiner's
(1626) plot detailing how configurations of sunspots change over time.
This technique would later be referred to as the ``principle of small
multiples'' by Tufte.}
...
\caption[John Snow's (1854) cholera map of
Soho.]{\label{fig-cholera}John Snow's (1854) map of cholera cases in
...
\caption[Charles Joseph Minard's (1869) flow diagram of Napoleon's
invasion of Russia.]{\label{fig-napoleon}Charles Joseph Minard's (1869)
flow diagram of Napoleon's botched invasion of Russia in 1812-1813. This
diagram shows Napoleon's advance and retreat on Moscow. The width of the
orange and black columns encodes the size of the Grande Armée. The
temperature scale on the lower portion of the graph illustrates the
weather conditions during the retreat, with a freezing Russian winter
causing high rates of attrition.}
...
\caption[Florence Nightingale's (1858) polar area
chart.]{\label{fig-nightingale}Florence Nightingale's (1858) polar area
...
Statistical Graphics'' \cite{friendly_2008b} saw the rise of forms of
...
\caption[Data visualisation milestones from Friendly (2008)
\cite{friendly_2008b}.]{\label{fig-rug-density}Major milestones in the
development of data visualisation illustrated using a rug plot and
density estimate. This figure is taken from Friendly (2008)
\cite{friendly_2008b}.}
...
\cite{friendly_2008b} in Figure~\ref{fig-rug-density}. Recounting a full
...
very high standard elsewhere \cite{friendly_2008a, friendly_2021}. Given
...
data relating to the measurement of different parts of the human body.
Early in this brief but significant paper, Galton puts forward a basic
...
product-moment correlation coefficient (Pearson's \emph{r}) in 1895
\cite{lee_1988} based on initial formulae by Bravais (1844)
\cite{bravais_1844}. This measure has been remarkably persistent,
remaining unchanged for well over a century. In fact, many other
measures of correlation, such as Spearman's \(\rho\), the point-biserial
correlation, and the \(\phi\) coefficient are actually special cases of
Pearson's \emph{r} applied to different types of data
\cite{henrysson_1971}; such is the dominance of the measure. Equation
2.1 defines Pearson's \emph{r}:
...
\cite{anscombe_1973}.]{\label{fig-anscombe}Anscombe's quartet
\cite{anscombe_1973}. Each scatterplot depicts datasets with identical
means (\(x\) = 9, \(y\) = 7.5), regression coefficients (\(y\) on \(x\)
= 0.5), standard errors (0.118), and Pearson's \emph{r} values (≈
0.816).}
...
\caption[Francis Galton's original plot comparing the semi-interquartile
ranges of stature (height) and cubit (forearm
length).]{\label{fig-galton-hand-plot}Francis Galton's original plot
...
\caption[Hertzsprung-Russell
diagram.]{\label{fig-HR-plot}Hertzsprung-Russell diagram of colour index
...
increase its utility in correlation visualisation. It is important,
however, to pay homage to the history of the visualisation as a valuable
tool for those willing and able to use graphing to solve scientific
problems.
...
\caption[Other types of correlation
visualisation.]{\label{fig-other-corr-viz}From top-left to bottom-right:
Parallel Coordinates Plot (PCP) built with the \texttt{GGally} package
(version 2.4.0 \cite{ggally}); Stacked area plot; radial stacked bar
plot (doughnut plot); radar plot. Each plot uses the same dataset with
an \emph{r} value of 0.6.}
...
\caption[Hexbin (left) and Kernel Density Estimation (KDE, right)
plots.]{\label{fig-remixes}Hexbin (left) and Kernel Density Estimation
...
correlation. I soon discovered that this was not the case; a whole host
of other visualisation types had been designed or adapted in an effort
to better visualise correlation. Parallel Coordinates Plots (PCPs)
\cite{heinrich_2013}, which are popular in the InfoVis community, are
most often used for multidimensional data. Applying them to bivariate
data results in plots that perform (almost) as well as scatterplots with
regards to visualising the correlation between two variables
\cite{kay_2016}. Figure~\ref{fig-other-corr-viz} illustrates PCPs, along
with three other types of correlation visualisation, with an \emph{r}
value of 0.6. Despite the existence of plots such as these, scatterplots
remain by far the most popular way of visualising correlation. With the
exception of the charts featured in Figure~\ref{fig-other-corr-viz},
many other ways of visualising correlation are density-flavoured remixes
of traditional scatterplots, such as Hexbin plots and Kernel Density
Estimate plots (see Figure~\ref{fig-remixes}). While there may be many,
often more visually appealing ways of visualising correlation,
traditional scatterplots offer a number of distinct advantages.

\subsection{Scatterplots}\label{scatterplots-corr-viz}

I begin this section by outlining common tasks that viewers of
scatterplots are expected to perform, including the types of visual
patterns typically involved. I then briefly discuss visual channel
rankings related to correlation perception and scatterplot tasks more
generally, before providing an overview of clutter and overplotting
issues, especially as they pertain to the techniques developed in this
thesis. Finally, I narrow my focus and discuss scatterplots specifically
as they relate to correlation perception.

\subsubsection{Tasks}\label{scatterplot-tasks}

Being a popular visualisation type, scatterplots have been used to
facilitate a wide range of visualisation tasks; Quadri \& Rosen
\cite{quadri_2022} provide a task-based overview of visualisations,
including scatterplots. Sarikaya \& Gleicher \cite{sarikaya_2018}
provide a task taxonomy specifically for scatterplots, and detail 12
abstract analysis tasks that may be applied to them. These abstract
analysis tasks are split into three types: \emph{object-centric} tasks,
such as value retrieval and comparison between data points;
\emph{browsing} tasks, such as finding clusters; and
\emph{aggregate-level} tasks, such as determining correlations or
anomalies. Kanjanabose et al.~\cite{kanjanabose_2015} used a set of four
tasks (value retrieval, clustering, outlier detection, and change
detection) when comparing task performance between scatterplots and
parallel coordinates plots, and Wang et al.~\cite{wang_2021} used three
(finding anomalies, judging correlation, and identifying clusters) when
studying data size and pattern salience with regards to scatterplot
comprehension. Lee et al.~\cite{lee_2017}, through development of the
Visualisation Literacy Assessment Test (VLAT), identify scatterplots as
being uniquely suitable for the eight visualisation tasks chosen
(retrieve value, find extremum, determine range, characterise
distribution, find anomalies, find clusters, find trends/correlations,
and make comparisons). Within a particular study or taxonomy, many tasks
overlap with regards to what viewers of visualisations actually do.
Hence, I focus on four higher level tasks, illustrated in
Figure~\ref{fig-task-examples}, for the remainder of this section:
...
\includegraphics[width=\textwidth]{2_related_work_files/figure-latex/fig-task-examples-1.pdf}

}

\caption[Examples of scatterplot
tasks.]{\label{fig-task-examples}Illustrating four different scatterplot
tasks using scatterplots similar to those used in the experiments in
this thesis. Visual elements are used to highlight the task detailed in
the title of each plot.}

\end{figure}%

Scatterplot tasks generally require viewers to engage with patterns in
the data. The value retrieval task usually requires viewers to use axis
ticks and labels to look up the \(x\) and \(y\) values for the
particular points they wish to locate. Such tasks may be facilitated by
increasing the salience of relevant data points, either as I have in
Figure~\ref{fig-task-examples} using additional visual scaffolding, or
by changing the size, colour, or shape of points relative to others.
Cluster separation, a task commonly associated with large-dataset
scatterplots, requires viewers to group points together based on their
similarities and differences with their neighbours. In
Figure~\ref{fig-task-examples}, I have used colour and marker shape to
delineate three clusters of points. The third task, characterising a
distribution, requires viewers to use the totality of points in the plot
to generate inferences about the structure of the data; when this
structure is clear, as in Figure~\ref{fig-task-examples}, this is a
relatively simple task, however becomes much more difficult as patterns
become more complex and obscured by noisy data. Finally, getting a sense
of correlation from a scatterplot also requires viewers to take into
account the shape of the underlying data; this thesis presents
visualisation techniques that are intended to aid in this.

\subsubsection{Visual Channels}\label{visual-channels}

Scatterplots are primarily used for presenting bivariate data based on
positions along \(x\) and \(y\) axes. Bubble charts (see Section
\ref{introduction-adjusting-size}, page
\pageref{introduction-adjusting-size}) encode an additional third
variable using point size. Aside from information overload, there is no
hard limit to the number of variables that a scatterplot might encode,
including the use of point size, marker shape, hue/gradient, or
orientation. Due to its suitability for encoding categorical data
\cite{kim_2018}, point colour is an often studied visual channel when
going past bivariate plots, however there is evidence that channels such
as colour, size, and shape are not entirely separable \cite{smart_2019}.
Such evidence means that design must take into account interactions
between visual channels, and motivates the study of point opacity and
size related to correlation perception from the bottom-up.

The property of correlation itself does not require visualisation with
scatterplots based on position values along an axis. Research has
investigated correlation visualisations that encode data point position
using line angles, areas, and orderings \cite{harrison_2014},
consistently finding that conventional, bivariate scatterplots offer the
best performance for visualising both positive and negative
correlations. Figure~\ref{fig-other-corr-viz} in Section
\ref{present-landscape-corr-viz} on page
\pageref{present-landscape-corr-viz} displays examples of other types of
correlation visualisation.

\subsection{Overplotting \& Clutter}\label{overplotting-clutter}

As the datasets that scatterplots describe increase in size and
complexity, overplotting becomes an issue. Increased amounts of data
results in increased amounts of point overlap, and reducing the size and
opacity of points are common ways of ameliorating these issues. As point
size and opacity decrease, however, individual point salience also
decreases, leading designers to be forced to consider a trade-off
between accurately representing the whole dataset and allowing for
individual data points to be effectively accessible to viewers
\cite{bertini_2004, matejka_2015}. To facilitate comparison across the
different experiments in this thesis, stimuli always featured 128
points, meaning the impact of overplotting was minimal. Some experiments
in this thesis, however, feature changes to the sizes of individual
points, resulting in overplotting and a reduction in the amount of
information a single data point can provide. Section \ref{ink} in
\chap{chap:gen_methods}, page \pageref{ink} contains a more
comprehensive discussion of the impact of overplotting throughout this
thesis, including visualisations that describe how much \emph{ink} was
actually present in experimental scatterplots once overlap had been
taken into account.

\subsection{Scatterplots \& Correlation
Estimation}\label{scatterplots-correlation-estimation}

In the context of correlation estimation from scatterplots, Weber's law
states that the differential change in perception between scatterplots
with two correlation values can be described by applying an
experimentally-derived constant (\(k\)) to the actual difference between
those correlations \cite{rensink_2010, harrison_2014}. This law may also
be applied to the discrimination of other physical properties, such as
weight \cite{coren_2004}. Replicating previous work investigating
Weber's law for the modelling of perceived correlation
\cite{rensink_2010}, Harrison et al.~\cite{harrison_2014} tested a range
of correlation visualisation types, including scatterplots and those
depicted in Figure~\ref{fig-other-corr-viz}, to investigate whether
precision in correlation estimation could be modelled by Weber's law
\cite{rensink_2010}. Participants were asked to make discriminative
judgements between side-by-side correlation visualisations and an
adaptive staircase procedure was employed to infer Just-Noticeable
Differences (JNDs) for correlation perception with nine correlation
visualisation types. Both positive and negative correlations were
tested. Fitting linear models to the JND data revealed that correlation
discrimination in the nine different visualisations tested
(scatterplots, PCPs, stacked area charts, stacked line charts, stacked
bar charts, donut charts, radar charts, line graphs, and ordered line
graphs) could be modelled using Weber's law. This study also provides a
ranking of the tested visualisation types with regards to participants'
correlation discrimination performance; overall, traditional
scatterplots outperformed all other visualisation types with positively
correlated data, and were tied with PCPs for negatively correlated data.
...
\caption[Comparing point cloud sizes with different \textit{r}
values.]{\label{fig-point-cloud-trends}Scatterplot with Pearson's
...
\(a\) is a constant coefficient and \(b\) is an exponent. \(X\)
represents the mean absolute vertical distance between the scatterplot
points and a regression line. This modelling provides further evidence
that estimates of correlation are based on perceptions of the physical
arrangement of scatterplot points. This finding received later support
from a study investigating the hypothesis that visual features may be
used to judge correlations in scatterplots \cite{yang_2019}. In that
study, it was found that, among other equally predictive visual
features, the standard deviation of all perpendicular distances between
scatterplot points and the regression line is predictive of performance
...
this quantity represents the mean distance between data points and the
regression line, and can be thought of as closely approximating the
...
\caption[Correlation estimation performance from previous
works.]{\label{fig-underestimation-curves}Using an equation supplied in
...
was also observed in Experiments 1 to 4 in the current thesis, and is
detailed in each experimental chapter. Figure \ref{fig-mean-all-exp} in
\chap{chap:conclusion}, page \pageref{fig-mean-all-exp} summarises the
mean estimation behaviour observed for each experiment throughout this
thesis. Despite this long-standing finding, no efforts have been made to
correct for this perceptual underestimation. Quite why this perceptual
bias correction has never been attempted, however, is unclear. The
following section argues in favour of seeking design additions to
correct for this.
...
\caption[BBC Covid visualisations.]{\label{fig-covid-viz}Two
visualisations taken from the BBC News article ``Covid: Twelve charts on
how Covid changed our lives'', published on the 15th of December, 2020
\cite{bbc_2020}.}

\end{figure}%

All this is to say that the underestimation of correlation matters. It
is not unreasonable to imagine that policy decisions have been made
based at least in part on data visualisations; the long-standing finding
that lay people and those educated in statistics underestimate the
levels of relatedness between variables displayed in scatterplots is not
trivial, and has real impact on people's lives. Those who design data
visualisations have a responsibility to design with \emph{all} viewers
in mind, not merely those who have made statistics a part of their
working lives. Designing in this way requires us to understand, on
...
\caption[Experimental interface for completing the Subjective Graph
Literacy scale \cite{garcia_2016}.]{\label{fig-sgl-screen}Participants
were instructed to use the following interface to complete the
Subjective Graph Literacy scale \cite{garcia_2016} before completing
...
and takes the form of five questions to which participants must provide
an answer. Generally, the scale ascertains each participant's
self-reported competencies when working with a small range of
visualisations. The SGL test portion of each experiment is shown in
Figure~\ref{fig-sgl-screen}.
...
\item
  Can these effects be used to correct for the underestimation bias?
\item
...
\tightlist
\item
  Changing the opacities and sizes of scatterplot points, both uniformly
  and using functions relating to residual magnitude, has strong effects
  on correlation estimation.
\item
  These effects can be used to correct for the underestimation bias
  without removing data, although significantly more work is needed to
  find the exact combinations of size and opacity adjustments that
  produce perceptually-optimised estimates.
\item
  These effects extend into a cognitive space, and are able to influence
  people's beliefs about the levels of relatedness between variables.
\item
  This project is exemplary of one done in an entirely open and
