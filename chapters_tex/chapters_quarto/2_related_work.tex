...
two variables \cite{kay_2015}. \textbf{?@fig-other-corr-viz} illustrates
parallel coordinates plots, along with 3 other types of correlation
visualisation, with an \emph{r} value of 0.6.
...
\caption{\label{fig-other-corr-viz-1}From left to right: Parallel
...
\includegraphics[width=\textwidth]{2_related_work_files/figure-latex/fig-other-corr-viz-2.pdf}

}

\caption{\label{fig-other-corr-viz-2}From left to right: Parallel
Coordinates Plot (PCP) built with the \texttt{GGally} package
\cite{ggally}; Stacked area plot; radial stacked bar plot (doughnut
plot); radar plot. Each plot uses the same dataset with an \emph{r}
value of 0.6.}
...
the charts featured in \textbf{?@fig-other-corr-viz}, many other ways of
...
those depicted in \textbf{?@fig-other-corr-viz}, to see if precision in
correlation estimation could be modelled by Weber's law
...
presented at 100, 400, or 1600 miliseconds. This study found that
performance was almost identical for scatterplots presented for either
400 or 1600 miliseconds, and that there was only a small deterioration
in performance for those presented for 100 miliseconds. The lack of
...
intuituve, perceptual nature of correlation perception in scatterplots.
