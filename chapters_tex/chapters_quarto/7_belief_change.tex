...
\ref{fig-estimates-by-r-e3}, \chap{chap:adjusting_size}, page
\pageref{fig-estimates-by-r-e3}). In
...
plots showing identical data. Through a pre-test and main experiment, I
...
\caption[Scatterplot manipulations from previous chapters and
corresponding estimation
behaviour.]{\label{fig-previous-manipulations}Top row: Examples of
scatterplot manipulations from previous work using an \textit{r} value
of 0.6. In each case, the manipulation used is non-linear, typical
orientation decay. Bottom row: the corresponding correlation estimation
behaviour across values of \textit{r} between 0.2 and 0.99. The dashed
diagonal line represents hypothetically accurate estimation, while the
solid line is the mean of what is observed when participants are asked
...
value of \(b\) (0.25), and the size scaling factor, additional constant,
and opacity floor. For the alternative scatterplot condition here, the
manipulation which has been previously shown to cause the most dramatic
change in participants' estimates of correlation was used: the
...
uncertainty visualisations. Accordingly, the pre-test is essential for
...
Both experiments in this chapter were conducted according to the
...
the pre-test\footnote{https://gitlab.pavlovia.org/Strain/beliefs\_scatterplots\_pretest}
...
\section{Pre-Test: Investigating Beliefs About Relatedness
...
perceptual effects on participants and their performance on
visualisation tasks.

\subsection{Materials}\label{materials-e5a}

I was unable to find resources for correlative statements that included
ratings for belief strength and emotional valence, so elected to create
my own. To control for these factors as much as possible, the pre-test
was run with the intent of finding a correlative statement that was
matched on emotional valence and level of belief strength. As opposed to
manually creating a list of candidate statements, I used the ChatGPT4
Large Language Model (LLM) \cite{chatGPT}. On the 9th April, 2024,
ChatGPT was asked:
...
\ref{chap:appendix_A}, page \pageref{chap:appendix_A}. A co-author and I
rated each statement on emotional valence and belief about strength of
relatedness using Likert scales from 1 to 7. Both statement emotional
valence and strength of relatedness were anchored at points 1 and 7:
\emph{Very Negative} and \emph{Very Positive} for the former, and
\emph{Not Related At All} and \emph{Strongly Related} for the latter.
All other points were unlabelled. The \texttt{irr} (version 0.84.1
\cite{irr}) package was used to calculate a quadratic weighted Cohen's
Kappa between the raters, which penalises larger magnitude disagreements
more harshly. Following each statement, a pair of Likert scales were
presented labelled ``Statement Emotionality'' and ``Strength of
Relatedness''. We agreed above chance for both emotional valence
(\(\kappa\) = 0.49, \emph{p} \textless{} .001) and strength of
relatedness (\(\kappa\) = 0.51, \emph{p} \textless{} .001), indicating
moderate levels of agreement in both cases
\cite{cohen_1968, fleiss_1969}. Together, we selected strongly and
weakly correlated statements with the highest levels of absolute
agreement, resulting in 14 strongly and 11 weakly correlated statements.
These statements are reproduced in Table~\ref{tbl-all-statements}.
...
with a representative UK population in order to ascertain consensus.

\subsection{Method}\label{method-e5a}

\subsubsection{Design}\label{design-e5a}

The pre-test featured a within-subjects design. Each participant saw all
experimental and attention check items in a fully randomised order.
...
In the experiment, participants were asked to read the 25 correlative
statements shown in Table~\ref{tbl-all-statements} and to indicate their
beliefs about topic emotionality and strength of relatedness. Stimuli
were presented in a randomised order, with six attention checks
interleaved. A screenshot of an experimental trial can be seen in
Figure~\ref{fig-e5-trial-example}.

\begin{figure}

\centering{

\includegraphics[width=\textwidth]{../supplied_graphics/e5_pre_test_trial.png}

}

\caption[An example of an experimental trial in the pre-test for
Experiment 5.]{\label{fig-e5-trial-example}An example of an experimental
trial in the pre-test, including the text stimulus participants were
asked to read, and the Likert scales they were asked to use to indicate
their beliefs about topic emotionality and strength of relatedness.}
...
mean age was 41.1 (SD = 12.3). The mean time taken to complete the
pre-test was 14.2 minutes (SD = 2.9 minutes).
...
relatedness (\(\kappa\) = 0.06, \emph{p} \textless{} .001). Results for
all 25 statements are shown in Figure~\ref{fig-all-statements-results}
on page \pageref{fig-all-statements-results}.

\begin{figure}

\centering{

\includegraphics[width=\textwidth]{7_belief_change_files/figure-latex/fig-all-statements-results-1.pdf}

}

\caption[Dot plots showing mean ratings of strength of relatedness and
emotionality across the 25 tested statements in the Experiment 5
pre-test.]{\label{fig-all-statements-results}Dot plots showing mean
ratings of strength of relatedness and emotionality across the 25 tested
statements. The vertical dashed line represents the midpoint on both
Likert scales. Standard deviations are plotted as errorbars. See
Table~\ref{tbl-all-statements} for the full text of each statement.
Statement 22 was eventually selected for the main experiment.}
...
are statements 2, 10, 16, 22, and 23, which can be seen in
...
\tightlist
\item
  H\textsubscript{1}: there will be a significant difference between
  ratings of strength of relatedness made before and after participants
  viewed scatterplots in either the standard or alternative conditions.
\item
  H\textsubscript{2}: this difference will be greatest when participants
  are exposed to scatterplots in the alternative scatterplot condition.
\end{itemize}
...
these factors can be found in Section \ref{add-analyses-e5} on page
\pageref{add-analyses-e5}.
...
\caption[Experiment 5 example
stimuli.]{\label{fig-exp5-examples-chap7}Examples of the stimuli for
Experiment 5 using \textit{r} values of 0.2, 0.6, and 0.99. Group A saw
the alternative scatterplots presented on the left, while group B saw
the standard design on the right. Plots in the alternative condition
(left) used the combined point opacity and size decay functions
developed in \chap{chap:interactions_opacity_size} (see the ``Both
Manipulations'' panel in Figure~\ref{fig-previous-manipulations} on page
\pageref{fig-previous-manipulations}).}
...
\includegraphics[width=\textwidth]{7_belief_change_files/figure-latex/fig-exp5-examples-chap7-cont-1.pdf}

}

\caption{\label{fig-exp5-examples-chap7-cont}(Continued)}
...
each condition. Examples of stimuli using \emph{r} values of 0.2, 0.6,
and 0.99 for both the standard and alternative scatterplot conditions
can be seen in Figure~\ref{fig-exp5-examples-chap7}.
...
their group, along with four attention check items, in a fully
randomised order. The dependent variable was the level of belief change
induced by viewing the scatterplot visualisations, so participants were
tested on how strongly related they believed the variables described by
the correlative statement to be both \textbf{before} and \textbf{after}
...
included a description of scatterplots as depicting ``the level of
relatedness between variables'', and this relatedness often being
``measured on a scale between 0 (not related at all) and 1 (extremely
related)'', participants were given two practice trials; these trials
took the form of a full opacity trial from Experiment 1. Participants
were then asked to indicate their beliefs about emotional valence and
strength of relatedness described in the chosen correlative statement;
these data were captured using Likert scales identical to those
described in Section \ref{materials-e5a} on page
\pageref{materials-e5a}. After completing 45 experimental trials,
participants were then asked again, using the same Likert scales, to
indicate their belief about the strength of relatedness described in the
correlative statement. Interspersed among the experimental items were
four attention check trials which explicitly asked participants to set
the slider to 0 or 1. A flowchart of the experimental procedure is
provided in Figure~\ref{fig-exp5-flowchart}. Justification for the
inclusion of the defensive confidence and graph literacy scales, and for
collecting participants' views of the emotional content of the chosen
...
\includegraphics[width=\textwidth]{../supplied_graphics/exp5_flowchart.png}

}

\caption{\label{fig-exp5-flowchart}Flowchart depicting the experimental
...
took participants an average of 27 seconds (SD = 16 seconds).
...
this was the primary motivation behind performing the pre-test.
...
fluency were required. As in the pre-test, UK residency was required of
...
pre-test, or in any of the experiments described in
\chap{chap:adjusting_opacity}, \chap{chap:adjusting_size}, or
\chap{chap:interactions_opacity_size} were prevented from participating.
Data were collected from 77 participants in each condition. 2
participants failed more than 2 out of 4 attention check questions for
each condition, meaning their data were excluded per pre-registration
...
mean age was 39.3 (SD = 11.5). Participants' mean graph literacy score
was 21.3 (SD = 4.3) out of 30, their mean defensive confidence score was
43 (SD = 6.8) out of 60, and their mean rating of statement emotional
valence was 2.9 (SD = 1.3) on a 7-point Likert scale. On average,
participants took 14.2 minutes to complete the experiment (SD = 6.41).
...
\ref{model-construction}, \chap{chap:gen_methods}, page
\pageref{model-construction} for further details). Odds ratios and
...
experimental plots, with ratings made \textbf{after} viewing the
experimental items being higher. A likelihood ratio test revealed that
the model including time of rating as a predictor explained
significantly more variance than the null (\(\chi^2\)(1) = 8,046.95,
\emph{p} \textless{} .001). This model has random intercepts for
participants. Statistical testing providing support for this hypothesis
is shown in Table~\ref{tbl-rating-time}.
Figure~\ref{fig-descriptives-e5} shows means and dot plots for ratings
of strength of relatedness made before and after viewing scatterplots in
either the standard or alternative condition.
...
value is also supplied. The estimate corresponds to the difference in
ratings of strength of relatedness made before and after participants
viewed the experimental items.}
...
Odds ratios and equivalent Cohen's \emph{d} effect sizes are also shown.
Estimates correspond to the difference in ratings of strength of
relatedness made before and after viewing plots (Rating Time) and
between alternative and standard conditions (Condition). For the
interaction, the estimate corresponds to the difference-in-difference
between ratings made pre- and post-viewing for standard and alternative
scatterplots.}
...
\caption[Dot plots showing individual pre- and post-plot viewing ratings
of strength of relatedness for standard and alternative scatterplot
conditions.]{\label{fig-descriptives-e5}Dot plots showing individual
pre- and post-plot viewing ratings of strength of relatedness for
standard and alternative scatterplot conditions. Mean ratings are also
shown as points.}
...
\caption[Histograms illustrating the magnitudes of the difference
between pre- and post-plot viewing ratings of strength of relatedness in
Experiment 5.]{\label{fig-difference-descriptive}Histograms illustrating
the magnitudes of the difference between pre- and post-plot viewing
ratings of strength of relatedness for standard and alternative
scatterplots. Difference is calculated by subtracting the ratings made
before viewing plots from the ratings made after. Median values are
plotted as points.}
...
\caption[Differences in beliefs about strength of relatedness plotted
separately for graph literacy scores, defensive confidence scores, and
statement emotional valence.]{\label{fig-add-analyses-plots}Illustrating
how differences in beliefs about strength of relatedness change as a
function of participants' scores on the graph literacy test (left),
their scores on the defensive confidence test (centre), and their
ratings of statement emotional valence (right). Locally smoothed curves
with 95\% CI ribbons are shown separately for standard and alternative
scatterplot viewing conditions. Lower ratings of Difference in Beliefs
(\(y\) axis) corresponds to lower levels of belief change between pre-
and post-scatterplot viewing times.}
...
Figure~\ref{fig-add-analyses-plots}. Participants with higher scores on
a graph literacy test experienced smaller changes in their ratings of
strength of relatedness when viewing standard scatterplots. This is in
line with previous work suggesting that those with higher levels of
graph or visualisation literacy show better performance in inference
tasks related to visualisations \cite{canham_2010}, are more capable of
describing effects that visualisations aim to communicate
\cite{shah_2011}, and can preferentially attend to relevant features of
visualisations to a greater degree \cite{okan_2016}, than those with
lower levels of graph literacy. An opposing pattern of results is seen
when participants viewed alternative scatterplots designed to elicit
higher levels of belief change; difference in belief is mostly stable
across different levels of graph literacy. A potential explanations for
this phenomena include the presumably unfamiliar nature of the stimuli
in the alternative scatterplot condition causing participants to attend
to the stimuli differently, however further work would be required to
explore this in more detail.

Up to a point, all participants with higher scores on the defensive
...
in light of evidence \cite{albarracin_2004}. This effect has previously
...
\sim 36/60 on the defensive confidence scale. It is unclear why the
alternative scatterplot condition is associated with high levels of
belief change in those with high defensive confidence. Further work
...
An alternative (although not competing) explanation for the results seen
here comes from recent work on the incorporation of uncertainty
...
of directions (see Section \ref{future-work-chap8},
\chap{chap:conclusion}, page \pageref{future-work-chap8}).
...
be simple to accomplish. Similarly, the size and opacity decay
parameters utilised in the alternative scatterplot condition have
resulted in a large amount of point overlap as \emph{r} approaches 1,
meaning individual point discriminability suffered (see
Figure~\ref{fig-exp5-examples-chap7} on page
\pageref{fig-exp5-examples-chap7}). Further tuning of these decay
functions, with potential adjustment for overlap, would be required if
plots such as these were to be used in real-life visualisations.

To date, there has been no qualitative work performed on alternative
scatterplot designs such as those utilised here; it may be that any
perceptual or cognitive benefits are outweighed by distrust or
unfamiliarity with novel designs. The dependent variable in the main
experiment was represented to participants as a simple, blunt, 7-point
Likert scale. While I argue that this is not particularly problematic
given that the intention was to find an effect, future work may wish to
use techniques that provide further scope for analysis, such as the
graphical elicitation method developed by Karduni et al.
