...
\label{chap:interactions_opacity_size}, the term ``contrast'' was
changed to ``opacity''. In order to maintain consistency throughout this
thesis, the more up-to-date wording (opacity) is used, although I
discuss the issue of terminology below.
...
(see section in related work about ways of testing correlation
perception).
...
(more opaque) and larger points are more salient \cite{healey_2012}, the
...
\ref{graph-literacy-lit-review} of \chap{chap:related_work} Early
...
reproducible research. All data and analysis code for the original paper
are available on GitHub \footnote{https://github.com/gjpstrain/contrast\_and\_scatterplots}.
...
the full analysis (50.67\% male, 47.33 \% female, and 1.33\%
non-binary). Participants' mean age was 28.29 (\emph{SD} = 8.59). Mean
graph literacy score was 21.79 (\emph{SD} = 4.47). The mean time taken
to complete the experiment was 33 minutes (SD = 10 minutes).
...
Table~\ref{tbl-efs-e1} along with means and standard deviations. The
...
correlation change with the objective \emph{r} value in the plot. Points
represent mean errors in correlation estimation, and standard deviations
of error are provided as error bars. The dashed horizontal line
represents hypothetical perfect estimation. As reported in previous
literature (see Sec in related work), participants underestimated
\emph{r} in nearly all cases (revise).
...
158 participants. 7 failed more than 2 out of 6 attention check
...
were included in the full analysis (51.33\% male, 46.00 \% female, and
2.67\% non-binary). Participants' mean age was 27.05 (\emph{SD} = 7.37).
Mean graph literacy score was 21.71 (\emph{SD} = 4.06). The average time
...
values. 0.25 was chosen as the value of \emph{b}, as it was felt at the
time that this rendered plots that maintained point visibility while
...
\textless{} .001). Figure~\ref{fig-e1-estimates} shows the mean errors
...
to the size of a particular point's residual. In experiment 1, I showed
that, in contradiction with previous work, changing the opacity of all
points in a scatterplot can effect participants' performance on a
correlation estimation task. In experiment 2, I showed that varying
point opacity as a function of a point's residual distance is able to
significantly change participants' correlation estimates and partially
correct for a historic underestimation bias.
