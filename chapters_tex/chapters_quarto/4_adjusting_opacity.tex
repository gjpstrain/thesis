...
\subsection{Results}\label{analysis-adjusting-opacity-e1}
...
\caption{\label{tbl-efs-e1}Cohen's \textit{d} effect sizes (left) and
summary statistics (right) for levels of the opacity factor in
experiment 1. Each effect size is compared to the reference level, full
contrast (alpha = 1).}
...
\cite{rensink_2012}. Figure~\ref{fig-estimates-by-r-e1} shows how
participants' estimates of correlation change with the objective
\emph{r} value in the plot. As reported in previous literature (see Sec
in related work), participants underestimated \emph{r} in nearly all
cases (revise).
...
Table~\ref{tbl-efs-e1} (right), standard errors for each opacity factor
...
\cite{fechner_1948, varshney_2013}; despite there being equaal linear
...
present the only other account of experiments that directly test
...
\cite{rensink_2014} may have been insufficiently powered. With the large
sample size in the present work (n = 150), evidence for an effect has
been found. The experimental methodology utilised here is more
representative of the use of scatterplots in the wild, and is therefore
more suited to informing design as opposed to investigating the
mathematical relationship between real and perceived correlation. While
the effect is small, it demonstrates definitively that differences in
the opacities of scatterplot points \emph{can} affect estimates of
correlation in positively correlated scatterplots. From the results it
is unclear why lowering global opacity causes greater errors in
...
between 0.75 and 1. That significant differences in correlation
estimation between data-identical scatterplots with different global
point opacities however, suggests that this effect may be leveraged to
further improve participants' performances on a correlation estimation
task.
...
observed as a function of the spatial arrangement of point opacity. With
this in mind, it was hypothesised that:
...
underestimation curve found in previous work (see related work chap 3 or
something), intuition suggested employing a symmetrically opposing curve
(see the non-linear decay curve in Figure~\ref{fig-opposing-curve}) to
relate point opacity to residuals.
...
Equation 4.1 was used to non-linearly map residuals to ggplot alpha
values, where R is the residual of the point. 0.25 was chosen as the
value of \emph{b}, as it was felt at the time that this rendered plots
that maintained point visibility while also allowing a large enough
point opacity range that, if an effect was present, it was likely to be
found.
...
\subsection{Results}\label{analysis-adjusting-opacity-e2}
...
\caption{\label{tbl-efs-e2}Cohen's \textit{d} effect sizes (left) and
summary statistics (right) for the opacity decay function factor in
...
separately for each value of \emph{r} used. The dotted horizontal line
represents perfect estimation, and standard deviations of estimates are
provided by way of error bars. Participants still underestimated
correlation in all conditions, although the use of the non-linear decay
function biased participants' estimates upwards to partially correct for
the underestimation.
...
Both hypotheses received support from experiment 2. Participants' errors
...
\emph{r} = 0.2, 0.5, 0.8, and 0.99 (see
...
correlation estimation could be attribute to this training, models were
...
models to the origin revealed a significant effect in experiment 1
...
useful. The methods used to not allow for investigation of absolute
...
opacity decay function demonstrated here may be able to accomplish this.
...
as bases for setting for opacity with regard to correlation estimation.
...
the literature. Chapter 5 investigates the use of point size in place of
opacity with the techniques described, while Chapter 6 combines the two
to explore how these different modalities work together with regards to
