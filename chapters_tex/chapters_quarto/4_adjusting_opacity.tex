...
experiments described here, however see Section \ref{ink} for plots
visualising the impacts of point overlap throughout this thesis. Due to
this, the approach to opacity described in this chapter would not be
useful when dealing with much larger datasets where clutter becomes an
issue.
...
residual magnitude using and inverted form of equation 4.1. Examples of
...
more generally was important. Given the lack of previous evidence that
correlation perception might be influenced by point opacity, I could not
hypothesise whether or not this might be the case. However, owing to the
robust effects of altering stimulus opacity on perception described in
Section \ref{related-work-adjusting-opacity}
...
mean age was 28.3 (\emph{SD} = 8.6). Mean graph literacy score was 21.8
(\emph{SD} = 4.5) out of 30. The mean time taken to complete the
...
participants underestimated \emph{r} in nearly all cases.
...
values in each condition using \emph{r} values of 0.2, 0.6, and 0.99.
...
non-binary). Participants' mean age was 27.1 (\emph{SD} = 7.4). Mean
graph literacy score was 21.7 (\emph{SD} = 4.1) out of 30. The mean time
...
to residuals. An minimum alpha value of 0.025 was used to ensure that
all points were visibly rendered, however, unlike in later chapters,
visibility testing was not carried out.
