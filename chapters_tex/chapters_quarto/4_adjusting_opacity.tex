...
as a function of residual magnitude is able to alter estimates
...
geometric scaling \cite{wei_2020}, the growing prevalence of data
visualisations in lay people's daily lives due to the COVID-19 pandemic,
and the increasing adoption of wearable devices, which by their nature
result in users viewing visualisations of a range of sizes
\cite{shandhi_2024}. The pilot experiment conducted therefore examined
how perceptions of correlation changed according to the size of a
scatterplot. Additionally, a very early version of the opacity decay
condition from Experiment 2 was included, however the implementation of
this condition was immature. Throughout this thesis, \textbf{decay}
refers to the way in which the opacity or size of a point is changed
according to its distance from the regression line; a \textbf{decay
function} refers, therefore, to the mathematical relationship between a
point's opacity or size and its residual magnitude.

In Experiment 2 onwards, if a scatterplot point resided in a particular
place on a scatterplot, it would always have the same opacity or size.
In the pilot study, the code that set the opacity of each point always
scaled the opacity values such that the point with the highest residual
had the lowest possible opacity, and vice versa, resulting in plots of
the type seen on the left in Figure~\ref{fig-pilot-study-examples}.
...
\caption[Examples of the experimental stimuli used in the pilot
study.]{\label{fig-pilot-study-examples}Examples of the experimental
stimuli used in the pilot study (opacity decay factor). On the left, the
opacity decay function is visible. Note the linear scaling and full
opacity range used.}

\end{figure}%

This provided no consistency between different experimental stimuli,
making it difficult to comment on the effects of changing the levels of
opacity in various parts of a plot on correlation estimation behaviour.
This was later addressed by hardcoding residual magnitude to a specific
value of opacity or size. The pilot also suffered extensively from poor
data quality. Of the 260 participants tested, data from only 118 was
included in the final analyses due to failed attention checks. It is for
this reason that the pre-screen requirements detailed in Section
\ref{recruitment} on page \pageref{recruitment} were implemented.

In the pilot experiment, participants viewed 180 experimental plots in a
\(3\times2\) factorial design. The first independent variable, plot
size, had three levels; 63\%, 100\%, and 252\% scales. The second was
the presence or absence of the opacity decay function (see
Figure~\ref{fig-pilot-study-examples}). I aimed to recruit 150
participants, but stopped after 118 due to ongoing data quality issues.
Nevertheless, the results provided were crucial in informing the future
direction of the research project. I present these results in brief
below.
...
opacity decay function (b) in the pilot study.}
...
factor (a) and opacity decay factor (b) in the pilot study.}
...
\cite{wei_2020}. Despite the statistical significance of this finding, I
elected at this point to abandon the plot size factor due to the
...
could be used systematically to correct for the historical
...
and lowering point opacity as a function of the magnitude of a point's
residual is able to bias estimates of positive correlation upwards to
partially correct for a historical underestimation bias (Experiment 2).
...
\caption[Demonstrating overplotting as it relates to point
opacity.]{\label{fig-overplotting-examples}Adjusting point opacity to
...
experiments described here, however see Section \ref{ink} on page
\pageref{ink} for plots visualising the impacts of point overlap
throughout this thesis. Due to this, the approach to opacity described
in this chapter would not be useful when dealing with much larger
datasets where clutter becomes an issue.
...
\caption[Demonstrating the relationship between \texttt{ggplot2} alpha
values and point opacity.]{\label{fig-alpha-examples}The relationship
between alpha values and rendered point opacity. Higher alpha values
result in greater contrast between the foreground (scatterplot point)
and background. When alpha = 0, the foreground is ignored and the
...
Section \ref{scatterplots-corr-viz} in \chap{chap:related_work}, page
\pageref{scatterplots-corr-viz}).
...
It was found that participants' estimates of mean point position were
...
second experiment examines how changing point opacity as a function of
distance from the regression line affects perceived correlation. To
...
\ref{graph-literacy-related-work} of \chap{chap:related_work}, page
\pageref{graph-literacy-related-work}. Early piloting with a graduate
student in humanities suggested the potential for participants to be
unfamiliar with the visual nature of different values of Pearson's
\emph{r}. Participants were therefore shown examples of \emph{r} = 0.2,
0.5, 0.8, and 0.95 (see
Figure~\ref{fig-training-slide-adjusting-opacity}); a discussion of the
effects of this training is provided in Section
\ref{training-adjusting-opacity} on page
\pageref{training-adjusting-opacity}. Participants were given two
practice trials to familiarise themselves with the response slider.
...
\caption[Training slide shown to
participants.]{\label{fig-training-slide-adjusting-opacity}Participants
viewed these plots for at least eight seconds before being allowed to
continue to the practice trials.}
...
all 4 (factor levels) \(\times\) 45 (\emph{r} values) = 180 experimental
items, corresponding to \sim 27,000 individual judgements per
experiment, in a fully randomised order.

\subsection{Materials}\label{shared-materials-chap4}

Both experiments used the same 45-\emph{r} value dataset. This dataset
was generated using a uniform distribution for 0.2 \textless{} \emph{r}
\textless{} 0.99. Each experiment featured a single factor. In
Experiment 1, this was the alpha value applied to each individual point,
and had four levels; alpha = 0.25, 0.5, 0.75, and 1 (full opacity).
Examples of stimuli in Experiment 1 can be seen in
Figure~\ref{fig-exp1-examples-chap4}. In Experiment 2, the factor was
the implementation of the point opacity decay function, and had four
levels; full opacity, linear decay, in which point opacity is decreased
linearly with residual magnitude, non-linear decay, in which point
opacity is decreased with residual magnitude using equation 4.1, and
inverted non-linear decay, in which point opacity is increased with
residual magnitude using an inverted form of equation 4.1. Examples of
the stimuli used in Experiment 2 can be seen in
Figure~\ref{fig-exp2-examples-chap4}.
...
original computational environment the paper was written in. Experiments
...
pre-registrations\footnote{Experiment 1 - https://osf.io/tuexh.
  Experiment 2 - https://osf.io/6f5ev}. There were no deviations from
the pre-registrations. It is important to note at this point that
Experiment 2 was conducted prior to Experiment 1; when the original
paper was written, the order of presentation of the experiments was
swapped to make the narrative more cohesive. I preserve this order in
the present chapter.
...
Findings from the pilot described above provided evidence against this
conclusion, so before proceeding with the fine-tuning of the immature
point opacity decay function, I felt that gaining an understanding of
how point opacity and correlation estimation interact more generally was
important. Given the lack of previous evidence that correlation
perception might be influenced by point opacity, I could not hypothesise
whether or not this might be the case. However, owing to the robust
effects of altering stimulus opacity on perception described in Section
\ref{related-work-adjusting-opacity}, page
\pageref{related-work-adjusting-opacity}
\cite{wehrhahn_1990, champion_2017}, it was hypothesised that:

\begin{itemize}
\tightlist
\item
  H\textsubscript{1}: A greater spread of estimates of correlation,
  measured by standard deviations, for plots with lower global opacity
...
the full analysis (77 male, 71 female, and 2 non-binary). Participants'
mean age was 28.3 (SD = 8.6). Mean graph literacy score was 21.8 (SD =
4.5) out of 30. The mean time taken to complete the experiment was 33
minutes (SD = 10 minutes).
...
demonstrated with \emph{r} values of 0.2, 0.6, and 0.99.
...
\caption[Examples of the stimuli used in Experiment
1.]{\label{fig-exp1-examples-chap4}Examples of the stimuli used in
Experiment 1, demonstrated with \textit{r} values of 0.2, 0.6, and 0.99.
Here, ``opacity'' refers to the alpha value used by \texttt{ggplot2}.}

\end{figure}%

\subsection{Results}\label{results-e1}

While I did not make hypotheses based on estimation accuracy, it is the
primary focus of the statistical analysis due to its relevance to later
work. To investigate the effects of opacity condition on participants'
estimates of correlation, a linear mixed effects model was built with
opacity condition as a predictor for the difference between objective
...
likelihood ratio test revealed that the model including global point
opacity as a fixed effect explained significantly more variance than a
null model (\(\chi^2\)(3) = 223.13, \emph{p} \textless{} .001).
...
\caption[Estimated marginal means of error for the four conditions
tested in Experiment 1.]{\label{fig-e1-estimates}Estimated marginal
means of error for the four conditions tested in Experiment 1. 95\%
confidence intervals are shown. The vertical dashed line represents no
estimation error. The overestimation zone is included to facilitate
...
be explained by differences in participants' levels of graph literacy,
an additional model was built. This model is identical to the
experimental model, but also includes graph literacy as a fixed effect.
Including graph literacy as a fixed effect explained no additional
variance (\(\chi^2\)(1) = .002, \emph{p} = .962), indicating that the
differences observed in participants' correlation estimation performance
were not as a result of differences in levels of graph literacy.
...
Each effect size is compared to the reference level, full opacity (alpha
= 1).}
...
\subcaption{\label{tbl-efs-e1-2}Means and standard errors of estimation
error.}
...
\caption[Participants' mean errors in correlation estimates grouped by
condition and by \textit{r}
value.]{\label{fig-estimates-by-r-e1}Participants' mean errors in
...
\emph{d} between the reference level (full opacity, alpha = 1.0) and
each other level of the opacity factor. These statistics can be seen in
Table \ref{tbl-efs-e1}(a) along with means and standard errors (b). The
largest effect size observed (\emph{d} \sim 0.16) is between the low and
full opacity conditions, and is small. This was unsurprising given the
lack of previously reported effects on correlation perception of global
point opacity \cite{rensink_2012}. Figure~\ref{fig-estimates-by-r-e1}
shows how participants' estimates of correlation change with the
objective \emph{r} value in the plot. The line shows mean errors in
correlation estimation, and standard deviations of error are provided as
error bars. The dashed horizontal line represents hypothetical perfect
estimation. As reported in previous literature (see Section
\ref{underestimation-related-work} in \chap{chap:related_work}, page
\pageref{underestimation-related-work}), participants underestimated
\emph{r} in nearly all cases.
...
scatterplot points was lower compared to when it was higher, however
accuracy is fairly consistent across experimental conditions. This held
true up until alpha was set to 0.75, implying a threshold around this
value past which there is little variation in the perception of opacity,
at least as far as it is associated with correlation estimation. This
lack of significant difference in correlation estimation between the two
...
points, and reports no difference in either bias (error) or variability
...
differences may have also played a role. Given the small effect size
(Cohen's \emph{d} = 0.16), the small sample (\emph{N} = 12) in Rensink
(2014) \cite{rensink_2014} may have caused the experiment to be
insufficiently powered. With the large sample size in the present work
(\emph{N} = 150), evidence for an effect has been found. The
experimental methodology utilised here is more representative of the use
of scatterplots in the wild, and is therefore more suited to informing
...
observed as a function of the spatial arrangement of point opacity. This
experiment builds on the immature opacity decay employed in the
unpublished pilot study (see Section \ref{pilot-study}). In that study,
point opacity was decreased as a function of residual magnitude, but the
range of opacity values used for each plot was the same. In the present
experiment, residual magnitude is hardcoded to point opacity, meaning
that if a point has a certain residual magnitude value, it will always
have the same opacity value. Additionally, in this experiment, I
introduce the concept of different types of decay. Linear decay refers
to conditions with a linear relationship between residual magnitude and
opacity. Non-linear decay conditions use equation 4.1 to map residual
magnitude to opacity values. Inverted decay conditions feature
scatterplots whose points increase in opacity as a function of residual
magnitude using an inverted form of equation 4.1. Due to alpha values
being hard coded to residual magnitudes, the full range of opacities is
not always accessed depending on the \emph{r} value and the particular
condition. To illustrate this, Figure \ref{fig-opposing-curve}
demonstrates the relationships between residual magnitudes and opacity
values in each condition using \emph{r} values of 0.2, 0.6, and 0.9.
...
\caption[Demonstrating the relationship between the size of a point's
residual and the alpha value (opacity)
rendered.]{\label{fig-opposing-curve}Using \textit{r} values of 0.2,
0.6, and 0.9 to demonstrate the relationship between the size of a
point's residual and the alpha value (opacity) rendered. Note that as
the \textit{r} value increases, the range of alpha values that can be
applied to points decreases.}

\end{figure}%

Given the previously reported effects of changing scatterplot point
opacity on other perceptual metrics (see Section
\ref{point-opacity-chap4} on page \pageref{point-opacity-chap4}), and
with the findings from Experiment 1 in mind, it was hypothesised that:

\begin{itemize}
\tightlist
\item
  H\textsubscript{1}: the non-linear decay parameter in which point
  opacity falls with residual magnitude will result in lower mean errors
  in correlation estimation compared to linear decay and full global
  opacity conditions.
\item
  H\textsubscript{2}: the use of the inverted non-linear decay
  parameter, in which point opacity becomes greater with residual
  magnitude, will result in higher mean errors in correlation estimation
  than for all other conditions.
\end{itemize}
...
non-binary). Participants' mean age was 27.1 (SD = 7.4). Mean graph
literacy score was 21.7 (SD = 4.1) out of 30. The mean time taken to
complete the experiment was 33 minutes (SD = 10 minutes).
...
demonstrated with \emph{r} values of 0.2, 0.6, and 0.99. Given the shape
of the underestimation curve found in previous work (see Figure
\ref{fig-underestimation-curves}, \chap{chap:related_work}, page
\pageref{fig-underestimation-curves}), intuition suggested employing a
symmetrically opposing curve (see the non-linear decay curve in
Figure~\ref{fig-opposing-curve}) to relate point opacity to residuals. A
minimum alpha value of 0.025 was used to ensure that all points were
visibly rendered, however, unlike in later chapters, visibility testing
was not carried out.

Equation 4.1 was used to non-linearly map residuals to \texttt{ggplot2}
alpha values. 0.25 was chosen as the value of \(b\), as it was felt at
the time that this rendered plots that maintained point visibility while
also allowing a large enough point opacity range that, if an effect was
present, it was likely to be found.

\begin{equation}
  point_{opacity} = 1 - b^{residual}
\end{equation}
...
\caption[Examples of the stimuli used in Experiment
2.]{\label{fig-exp2-examples-chap4}Examples of the stimuli used in
Experiment 2, demonstrated with \textit{r} values of 0.2, 0.6, and
0.99.}
...
\caption[Estimated marginal means of error for the four conditions
tested in Experiment 2.]{\label{fig-e2-estimates}Estimated marginal
means of error for the four conditions tested in Experiment 2. 95\%
confidence intervals are shown. The vertical dashed line represents no
estimation error. The overestimation zone is included to facilitate
comparison to later work.}
...
\subcaption{\label{tbl-efs-e2-2}Means and standard errors of estimation
error.}
...
Table~\ref{tbl-efs-e2}. The largest effect size observed (\sim 0.23
between full opacity and inverted decay conditions) is small to
moderate, and the effect size between the baseline and the non-linear
decay condition (\sim 0.19) is small. Figure~\ref{fig-estimates-by-r-e2}
illustrates the effects of each manipulation on participants'
correlation estimation performance separately for each value of \emph{r}
used. The dashed horizontal line represents perfect estimation, and
standard deviations of estimation error are provided by way of error
bars. Participants still underestimated correlation in all conditions,
although the use of the non-linear decay function biased participants'
...
\caption[Participants' mean errors in correlation estimates grouped by
condition and by \textit{r} value in Experiment
2.]{\label{fig-estimates-by-r-e2}Participants' mean errors in
correlation estimates grouped by condition and by \textit{r} value. The
dashed horizontal line represents perfect estimation. Participants were
most accurate when presented with the plots featuring the non-linear
opacity decay function (far left). Error bars show standard deviations
of estimates.}
...
in correlation estimation were lowest when the non-linear opacity decay
function was used, and were highest when scatterplots employed the
non-linear inverted decay function. There was no significant difference
in correlation estimation errors between the linear decay function and
full contrast conditions. This result was surprising, however on closer
...
with high opacity values (alpha \textgreater{} 0.75). This resulted in
no significant perceived differences between full opacity and linear
decay function scatterplots. A similar threshold for high opacity values
was found in Experiment 1. Selecting only lower \emph{r} values, those
with naturally higher residuals (arbitrarily \emph{r} \textless{} 0.6),
still results in no difference between correlation estimation errors for
linear decay parameters and full opacity conditions (\(\chi^2\)(1) =
0.09, \emph{p} = .769). Effect sizes were small, with the largest being
between the baseline full opacity and inverted non-linear decay
conditions. This suggests that it is easier to induce further bias in
correlation estimates through a reduction in the salience of a point
cloud's centre than it is to correct for the underestimation bias.
...
Experiment 1 (see Figure~\ref{fig-estimates-by-r-e1} on page
\pageref{fig-estimates-by-r-e1}), and aside from the inverted non-linear
decay function condition, precision in \emph{r} estimation increased
with the objective \emph{r} value. This finding corroborates previous
work. In the inverted non-linear decay condition, as \emph{r} approaches
1 (and point residuals accordingly approach 0), the opacity of points
diminishes. Just as the standard deviation of correlation estimates was
higher for the low global opacity condition in Experiment 1, having
lower opacity points at the high \emph{r} end of the non-linear inverted
condition in Experiment 2 resulted in fairly constant standard
deviations across \emph{r} values, as the usual reduction towards
\emph{r} = 1 was not observed.
...
opacity as a function of residual magnitude can partially correct for
this underestimation, and that raising opacity as a function of residual
magnitude can increase the underestimation bias. These findings pave the
way for using changes in point opacity to produce perceptually-optimised
...
to interpret and understand, I used a large, representative sample,
...
\item
  ...
  falls as a function of residual magnitude, can be used to counteract
  the underestimation seen in correlation estimation in positively
...
correlation displayed as accurately as possible. Of course, when
scatterplots are designed with more than simple correlation estimation
in mind, the techniques described here might not be suitable, and may
impede the completion of other scatterplot tasks. For example, if
viewers were also expected to be able to find outliers, the reduced
opacity of exterior points may hamper their ability to do so.
...
Figure~\ref{fig-training-slide-adjusting-opacity} on page
\pageref{fig-training-slide-adjusting-opacity}) to familiarise
...
presented sequentially with conventional ones. Testing whether this
phenomenon would exist with a plot in isolation would present a number
of difficulties. As can be seen in Figure~\ref{fig-estimates-by-r-e1}
and Figure~\ref{fig-estimates-by-r-e2}, participants' estimates of
...
limitations do not QFprevent the data gathered from being practically
...
The most exciting avenue for related future work is expanding the
techniques developed here to other visual features in isolation and
combination. \chap{chap:adjusting_size} investigates the use of point
size in place of opacity with the techniques described, while
\chap{chap:interactions_opacity_size} combines the two to explore how
these different modalities work together with regards to the estimation
of positive correlation.

\section{Conclusion}\label{conclusion-chap4}

In a pair of experiments, I varied the opacity of points in scatterplots
...
changing the opacity of all points in a scatterplot can affect
