...
for a portion of the total experimental items. In Experiments 1 to 4, each 
...
# use plot_example_function within ggarrange() to create examples of experimental plots
...
# use plot_example_function within ggarrange() to create examples of experimental plots
# do this separately for each class of plot, then combine with a second
# ggarrange() implementation
...
ggarrange(example_plot_function_exp5(slopes_exp5,
  ...
  # add labels for typical/atypical class
...
level of timings accuracy \cite{bridges_2020}. Using such an open-source tool not only
...
are significant differences in the means of the dependent variables between conditions.
...
Since then, containerisation software has experienced rapid development and uptake, mostly within the 
...
date packages here. Accordingly, a single implementation of Docker
