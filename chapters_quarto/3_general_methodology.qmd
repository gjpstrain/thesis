...
@fig-slider). Experiment 1 featured a single factor of global scatterplot point
...
system. Despite these findings, there has also been evidence of low data quality
and skewed demographics affecting both general crowdsourcing platforms, such as
...
The first experiment conducted was a pilot study (see Section \ref{pilot-study}
...
recruitment rate. \[\^1\]: this is a more strict rate than the 95% recommended
by Peer et al. \cite{peer_2021}.
...
ticks themselves were preserved. @fig-scatter-example demonstrates the basic
...
All analyses in this thesis were conducted using R (version 4.5.0
...
all experiments bar Experiment 5), the `emmeans` package \cite{lenth_2024} is
...
responses to Likert scale data \cite{liddell_2018}. In light of these issues,
the `ordinal` package \cite{ordinal} in R was used to build cumulative link
...
(whether or not a solution can be found), the `buildmer` package \cite{buildmer}
...
most complex model structure that successfully converges. Following this, random
effects terms that fail to explain a significant amount of variance in the
...
`r2glmm` package \cite{r2glmm} was used instead to calculate semi-partial R^2^.
...
the `effectsize` package \cite{effectsize}. More details on specific
...
the Dockerfile used to reproduce this thesis.

I first specify the Rocker image that will form the basis of the container. This
includes the version of R required (version 4.4.2), the Rstudio Integrated
...
[template](https://github.com/gjpstrain/UoM_reproducible_thesis_template) to
facilitate future reproducible theses. FAIR (**F**indable, **A**ccessible,
