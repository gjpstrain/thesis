...
INPUT_DIR  <- "../data/exp_plots/exp_1_all_plots/"
...
measure_ink <- function(path) {
  ...
  rgb <- img[, , 1:3]
  brightness <- (rgb[, , 1] + rgb[, , 2] + rgb[, , 3]) / 3
...
results %>%
  ...
  ggplot(aes(x = item, y = ink_total ##, fill = condition)) +
  )) +geom_col() +
  # scale_fill_manual(values = c(A = "#4393c3", B = "#74c476",
  #                              C = "#fd8d3c", D = "#e31a1c"),
                    # guide = "none") +
  facet_wrap(~ condition_label, nrow = 4) +
  labs(
    x     = "Plot number",
    y     = "Total ink"
  ) +
