...
labels_e1 <- c(A = "Low Opacity (alpha = 0.25)",
               B = "Medium Opacity (alpha = 0.5)",
               C = "High Opacity (alpha = 0.75)",
               D = "Full Opacity (alpha = 1.0)")
...
labels_e3 <- c(A = "Non-Linear Decay",
               B = "Linear Decay",
               C = "Inverted Decay",
               D = "Standard Size")

labels_e4 <- c(A = "Typical Orientation Size × Typical Orientation Opacity (**congruent**)",
               B = "Inverted Orientation Size × Inverted Orientation Opacity (**congruent**)",
               C = "Typical Orientation Size × Inverted Orientation Opacity (**incongruent**)",
               D = "Inverted Orientation Size × Typical Orientation Opacity (**incongruent**)")
```
...
#| fig-scap: Experimental trial example from Experiments 1 to 4.
#| out-width: NA
#| out-height: NA
...
provided a judgement of correlation using a sliding scale between 0 and 1 (see the
example of an experimental trial in @fig-exp-illustrate). Experiment 1 featured a single factor of global scatterplot point
...
featured a factorial $2\times2$ design; IV~1~ was the scatterplot point
...
a 1-factor, 2-level between-participants design; group A saw alternative scatterplots
designed to elicit greater levels of belief change compared to standard
scatterplots, which were shown to group B (see @fig-exp5-examples). Characteristics
of all experiments are summarised in @tbl-exp-summary, which also includes
locations for full descriptions of stimuli.
...
#| fig-scap: Examples of the stimuli used in Experiment 3.
#| fig-asp: 0.8
...
#| include: true
#| fig-cap: Examples of the stimuli for Experiment 5 using \textit{r} values of 0.2, 0.6, and 0.99. Group A saw the alternative scatterplot presented on the left, while group B saw the standard design on the right. 
#| fig-scap: Examples of the stimuli used in Experiment 5.
#| fig-asp: 1.5
#| out-width: "100%"
...
#| include: true
#| tbl-cap: Experimental summary table showing the visual channel manipulated, the type of manipulation, dependent variable, presence or absence of a baseline condition, experimental design, section, and chapter.

table_df <- read_csv("../data/exp_summary.csv")

kbl(table_df, booktabs = TRUE, escape = FALSE) %>%
...
Overlap between points was not taken into account during any of the experiments
described in this thesis. As *r* increases and points become closer together,
the amount of overlap also increases. Due to this, the total proportion of the
plot occupied decreases. I refer to this concept as the total amount of *ink*
present in a particular plot. Below, I present visualisations of the amount of
ink present in a scatterplot, and how this changes as a function of the *r* value the plot
describes. These visualisations take into account point opacity and point 
overlap. The ink distribution plot for experiment 1 can be seen in @fig-ink-e1,
for experiment 2 in @fig-ink-e2, for experiment 3 in @fig-ink-e3, for experiment
4 in @fig-ink-e4, and for experiment 5 in @fig-ink-e5. In all cases, total ink
values only refer to the plotted scatterplot points in relation to the white plotting
background; all other visual features,
including axis lines and ticks in experiments 1 to 4, and text in experiment
5, are discounted. Each visualisation shows the normalised total ink
($\frac{total\ ink}{total\ pixel\ count}$) on the $y$-axis.

```{r}
#| label: fig-ink-e1
#| include: true
#| fig-cap: Changes in the total amount of ink in plots in experiment 1 as a function of \textit{r} value, plotted separately for the four global opacity conditions. Note how in each condition, the total amount of ink begins to drop very slightly as the objective \textit{r} value increases.
#| fig-scap: Total ink plot for Experiment 1.
#| fig-asp: 1

plot_ink <- function(input_dir, condition_labels,
                     row_end = 1132, col_start = 68) {
  
  set.seed(1234)

  my_rs <- seq(0.2, 0.99, length.out = 45)
  
  CONDITION_ALPHA <- c(A = 0.25, B = 0.50, C = 0.75, D = 1.00)
  
  measure_ink <- function(path) {
    img        <- readPNG(path)
    brightness <- (img[, , 1] + img[, , 2] + img[, , 3]) / 3
    darkness   <- 1 - brightness
    interior   <- darkness[1:row_end, col_start:ncol(darkness)]
    list(
      ink_total      = sum(interior),
      ink_normalised = mean(interior),
      n_pixels       = length(interior)
    )
  }
  
  png_files <- list.files(input_dir, pattern = "\\.png$", full.names = FALSE)
  
  parsed <- tibble(filename = png_files) %>%
    mutate(
      item      = str_extract(filename, "^\\d+"),
      condition = str_extract(filename, "(?<=\\d)[A-Da-d](?=\\.png$)") %>% toupper()
    ) %>%
    filter(!is.na(item), !is.na(condition))
  
  skipped <- setdiff(png_files, parsed$filename)
  if (length(skipped) > 0)
    message("Skipped ", length(skipped), " unrecognised file(s): ",
            paste(head(skipped, 5), collapse = ", "))
  
  results <- parsed %>%
    mutate(alpha = CONDITION_ALPHA[condition]) %>%
    mutate(metrics = map(
      file.path(input_dir, filename),
      \(p) { message("Processing: ", basename(p)); measure_ink(p) }
    ))
  results <- unnest_wider(results, metrics)
  
  n_items <- max(as.integer(results$item))
  tick_pos <- seq(1, n_items, length.out = 10)

  results %>%
    mutate(item = as.integer(item)) %>%
    ggplot(aes(x = item, y = ink_normalised, fill = condition)) +
    geom_col() +
    facet_wrap(~ condition, nrow = 4,
               labeller = labeller(condition = condition_labels)) +
    scale_x_continuous(
      breaks = tick_pos,
      labels = round(my_rs[tick_pos], 2)
    ) +
    #scale_y_continuous(limits = c(0, 40000)) +
    scale_fill_brewer(palette = "Dark2") +
    labs(x = "*r* value", y = "Normalised Total Ink") +
    theme_ggdist() +
    theme(
...
plot_ink(
  input_dir         = "../data/exp_plots/exp1_all_plots/",
  condition_labels  = labels_e1
)
```
...
#| label: fig-ink-e2
#| include: true
#| fig-cap: Changes in the total amount of ink in plots in experiment 2 as a function of \textit{r} value, plotted separately for the three opacity decay conditions and the full opacity condition.
#| fig-scap: Total ink plot for Experiment 2.
#| fig-asp: 1


plot_ink(
  input_dir         = "../data/exp_plots/exp2_all_plots/",
  condition_labels  = labels_e2
)
```
...
#| label: fig-ink-e3
#| include: true
#| fig-cap: Changes in the total amount of ink in plots in experiment 3 as a function of \textit{r} value, plotted separately for the three size decay conditions and the standard size condition. The influence of point overlap causes the total amount of ink to fall in all conditions, with non-linear decay experiencing some rise in total ink prior.
#| fig-scap: Total ink plot for Experiment 3.
#| fig-asp: 1


plot_ink(
  input_dir         = "../data/exp_plots/exp3_all_plots/",
  condition_labels  = labels_e3,
  row_end = 993, col_start = 62
)
```
...
#| label: fig-ink-e4
#| include: true
#| fig-cap: Changes in the total amount of ink in plots in experiment 4 as a function of \textit{r} value, plotted separately for the four opacity and size decay conditions.
#| fig-scap: Total ink plot for Experiment 4.
#| fig-asp: 1

plot_ink(
  input_dir         = "../data/exp_plots/exp4_all_plots/",
  condition_labels  = labels_e4,
  row_end = 993, col_start = 62
)
```
...
#| label: fig-ink-e5
#| include: true
#| fig-cap: Changes in the total amount of ink in plots in experiment 5 as a function of \textit{r} value, plotted separately for standard and alternative plotting conditions. Only the scatterplot points themselves are included in these calculations.
#| fig-scap: Total ink plot for Experiment 5.
#| fig-asp: 0.5

# new ink plotting function needed for exp 5

labels_e5 <- c("A" = "Alternative Scatterplot",
               "T" = "Standard Scatterplot")

plot_ink_e5 <- function(input_dir, condition_labels,
                     row_end = 1132, col_start = 68,
                     row_start = 1, col_end = NULL,
                     conditions = c("A", "T"),
                     ink_threshold = 0) { 
  
  set.seed(1234)
  
  my_rs <- seq(0.6, 0.99, length.out = 45)
  
  CONDITION_ALPHA <- setNames(seq(0.25, 1, length.out = length(conditions)), conditions)
  
  measure_ink <- function(path) {
    img        <- readPNG(path)
    brightness <- (img[, , 1] + img[, , 2] + img[, , 3]) / 3
    darkness   <- 1 - brightness
    col_max    <- if (is.null(col_end)) ncol(darkness) else col_end
    interior   <- darkness[row_start:row_end, col_start:col_max]
    interior[interior <= ink_threshold] <- 0
    list(
      ink_total      = sum(interior),
      ink_normalised = mean(interior),
      n_pixels       = length(interior)
  ...
  cond_pattern <- paste0("[", paste(c(conditions, tolower(conditions)), collapse = ""), "]")
  
  parsed <- tibble(filename = png_files) %>%
  mutate(
    item      = str_extract(filename, "^\\d+"),
    condition = str_extract(filename, "(?<=_spice_)[A-Ta-t](?=_)") %>% toupper()
  ) %>%
  filter(!is.na(item), !is.na(condition))
  ...
  n_items  <- max(as.integer(results$item))
  tick_pos <- round(seq(1, n_items, length.out = 10))
...
plot_ink_e5(
  input_dir        = "../data/exp_plots/exp5_all_plots",
  condition_labels = labels_e5,
  conditions       = c("A", "T"),
  row_start        = 272,
  row_end          = 1288,
  col_start        = 34,
...
study (see Section \ref{pilot-study} on page \pageref{pilot-study}) by preventing participants who joined
...
on page \pageref{pilot-study} for full details) investigating a very early iteration of the point opacity
...
the next experiment fell to \sim 5%. Rejection rates were similar for the remainder
...
*d* \sim 0.2), and aimed to recruit enough participants to adequately power the
...
#| fig-scap: Visualising random intercepts and slopes for mixed effects models.
#| out-width: NA
#| out-height: NA
#| fig-asp: 0.6
...
present in the datasets. In order to ascertain the goodness-of-fit of models,
...
otherwise be a makeshift script followed by copy-pasting results into
...
files required, and the correct package versions used during analysis. The
`renv` package (version 1.1.8 \cite{renv}) is used to capture package versions
and dependencies.
...
includes the version of R required (version 4.5.3), the Rstudio Integrated
Development Environment (IDE), Quarto, and the `tidyverse` package.

`FROM rocker/version:4.5.3`

Then, I add the required `renv` files that I have previously used to capture
...
Following that, I instruct `renv` to use the `renv.lock` file it generated to
install the exact versions of the packages the project requires:

`RUN R -e "renv::restore()"`

Next, I add the files and folders required, including the Quarto setup file,
chapter folder, bibliography, additional scripts, LaTeX class
file and template, the folders containing the raw data and cached models, and
the R project file:
...
Finally, I use `tinytex` to install necessary additional LaTeX files that
are required to produce the final document. For brevity, I only show the first
few here:
...
crisis, ranging from poor practice (e.g. Potti et al., 2006)
...
#| fig-scap: Peng's (2011) Reproducibility Spectrum.
#| out-width: NA
#| out-height: NA
#| fig-asp: 0.2
