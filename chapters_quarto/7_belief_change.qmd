...
\ref{fig-estimates-by-r-e3}, \chap{chap:adjusting_size}, page \pageref{fig-estimates-by-r-e3}). In
...
plots showing identical data. Through a pre-test and main experiment, I provide
...
#| include: true
#| fig-cap: "Top row: Examples of scatterplot manipulations from previous work using an \\textit{r} value of 0.6. In each case, the manipulation used is non-linear, typical orientation decay. Bottom row: the corresponding correlation estimation behaviour across values of \\textit{r} between 0.2 and 0.99. The dashed diagonal line represents hypothetically accurate estimation, while the solid line is the mean of what is observed when participants are asked to estimate correlation."
#| fig-scap: Scatterplot manipulations from previous chapters and corresponding estimation behaviour.
#| fig-pos: "H"
...
plot_example_function_special <- function (df, t, o, s, title_size) {
...
example_plots_prev <- ggarrange(
                      plot_example_function_special(slopes,
...
utilised here, including the number of points (*n* = 128), the value of $b$ (0.25),
and the size scaling factor, additional constant, and opacity floor. For
...
visualisations. Accordingly, the pre-test is essential for isolating the
...
Both experiments in this chapter were conducted according to the principles of
open and reproducible research \cite{ayris_2018}. All experimental code,
materials, and instructions are hosted on GitLab for the pre-test[^1] and for
...
# Pre-Test: Investigating Beliefs About Relatedness Statements {#beliefs-e5a}
...
their performance on visualisation tasks.

## Materials {#materials-e5a}

I was unable to find resources for correlative statements
that included ratings for belief strength and emotional valence, so elected to
create my own. To control for these factors as much as possible, the pre-test
...
The full list of statements can be found in Appendix \ref{chap:appendix_A},
page \pageref{chap:appendix_A}. A co-author and I
...
#| include: true
#| tbl-cap: Statements tested with participants in the pre-test.
...
population in order to ascertain consensus.

## Method {#method-e5a}

### Design {#design-e5a}

The pre-test featured a within-subjects design. Each participant saw all experimental
and attention check items in a fully randomised order.
...
In the experiment, participants were asked to read the 25 correlative statements
shown in @tbl-all-statements and to indicate their beliefs about topic emotionality
and strength of relatedness. Stimuli were presented in a randomised order, with six
attention checks interleaved. A screenshot of an experimental trial can be seen
in @fig-e5-trial-example.

```{r}
#| label: fig-e5-trial-example
#| include: true
#| fig-cap: An example of an experimental trial in the pre-test, including the text stimulus participants were asked to read, and the Likert scales they were asked to use to indicate their beliefs about topic emotionality and strength of relatedness.
#| fig-scap: An example of an experimental trial in the pre-test for Experiment 5.
#| out-width: NA
#| out-height: NA
...
knitr::include_graphics(path = "../supplied_graphics/e5_pre_test_trial.png", dpi = NA)
```
...
taken to complete the pre-test was `r printnum(time_taken_e5$mean, digits = 1)`
...
statements are shown in @fig-all-statements-results on page \pageref{fig-all-statements-results}.

```{r}
#| label: fig-all-statements-results
#| include: true
#| fig-asp: 1.5
#| fig-cap: Dot plots showing mean ratings of strength of relatedness and emotionality across the 25 tested statements. The vertical dashed line represents the midpoint on both Likert scales. Standard deviations are plotted as errorbars. See @tbl-all-statements for the full text of each statement. Statement 22 was eventually selected for the main experiment.
#| fig-scap: Dot plots showing mean ratings of strength of relatedness and emotionality across the 25 tested statements in the Experiment 5 pre-test.
...
summary_df <- E5_beliefs_pre_test_tidy %>%
  ...
  ggplot(plot_df,aes(x = mean, y = statement, color = measure, shape = measure)) +
    geom_vline(xintercept = 3.5, linetype = 5, colour = "gray70") +
    geom_pointrange(aes(xmin = mean - sd, xmax = mean + sd),
                 position = position_dodge(width = 0.6)) +
  geom_point(size = 2, position = position_dodge(width = 0.5)) +
  scale_x_continuous(breaks = 1:7, limits = c(1, 7.352)) +
  ...
  scale_shape_manual(values = c("corr" = 16, "emot" = 17),
                     labels = c("Strength of Relatedness", "Emotional Valence")) +
  labs(x = "Mean\nRating", y = NULL, color = NULL, shape = NULL) +
  theme_ggdist() +
...
-   H~1~: there will be a significant difference between ratings of strength of
    relatedness made before and after participants viewed scatterplots in either
    the standard or alternative conditions.
-   H~2~: this difference will be greatest when participants are exposed to
...
\ref{add-analyses-e5} on page \pageref{add-analyses-e5}.
...
#| include: true
#| fig-cap: Examples of the stimuli for Experiment 5 using \textit{r} values of 0.2, 0.6, and 0.99. Group A saw the alternative scatterplots presented on the left, while group B saw the standard design on the right. Plots in the alternative condition (left) used the combined point opacity and size decay functions developed in \chap{chap:interactions_opacity_size} (see the "Both Manipulations" panel in @fig-previous-manipulations on page \pageref{fig-previous-manipulations}). 
#| fig-scap: Experiment 5 example stimuli.
#| fig-asp: 1.5
#| out-width: "100%"
...
ggarrange(plots_02, plots_06, ncol = 1)
```
...
```{r}
#| label: fig-exp5-examples-chap7-cont
#| fig-cap: "(Continued)"
#| fig-asp: 0.75
#| include: true
...
Examples of stimuli using *r* values of 0.2, 0.6, and 0.99 for both the standard and
...
viewed alternative scatterplots designed deliberately to elicit higher levels 
of belief change, or group B, in which they viewed standard scatterplots.
Participants saw all 45 experimental items for their group, along with four
...
described by the correlative statement to be both **before** and **after**
...
Following instructions, which included a description of scatterplots as depicting
"the level of relatedness between variables", and this relatedness often being
"measured on a scale between 0 (not related at all) and 1 (extremely related)", 
participants were given two practice trials; these trials took
...
data were captured using Likert scales identical to those described in Section
\ref{materials-e5a} on page \pageref{materials-e5a}.
...
described in the correlative statement. Interspersed among the 
experimental items were four attention check trials which explicitly 
asked participants to set the slider to
0 or 1. A flowchart of the experimental procedure is provided in @fig-exp5-flowchart.
Justification for the inclusion of the defensive confidence and graph
literacy scales, and for collecting participants' views of the emotional content
of the chosen statement, are provided below.

```{r}
#| label: fig-exp5-flowchart
#| include: true
#| fig-cap: Flowchart depicting the experimental procedure for Experiment 5.
#| out-width: NA
#| out-height: NA
# find exp example png in supplied graphics folder
...
knitr::include_graphics(path = "../supplied_graphics/exp5_flowchart.png", dpi = NA)
```
...
the primary motivation behind performing the pre-test. Nevertheless, it is not
...
the pre-test, UK residency was required of participants, as the experiment
relied on familiarity with the visual style of a British news source.
Participants who took part in the pre-test, or in any of the experiments
...
\ref{model-construction}, \chap{chap:gen_methods}, page \pageref{model-construction}
for further details). Odds ratios and equivalent
...
participants viewed the experimental plots, with ratings made **after** viewing
the experimental items being higher. A likelihood ratio test revealed
...
#| include: true
#| tbl-cap: "Statistics for the significant main effect of rating time. Odds ratio and the equivalent Cohen's \\textit{d} value is also supplied. The estimate corresponds to the difference in ratings of strength of relatedness made before and after participants viewed the experimental items."
...
#| include: true
#| tbl-cap: "Statistics for the significant main effect of rating time and the significant interaction between rating time and condition on the difference between pre- and post-scatterplot viewing ratings for standard and alternative plots. Odds ratios and equivalent Cohen's *d* effect sizes are also shown. Estimates correspond to the difference in ratings of strength of relatedness made before and after viewing plots (Rating Time) and between alternative and standard conditions (Condition). For the interaction, the estimate corresponds to the difference-in-difference between ratings made pre- and post-viewing for standard and alternative scatterplots."
...
#| include: true
#| fig-asp: 0.6
#| fig-cap: Dot plots showing individual pre- and post-plot viewing ratings of strength of relatedness for standard and alternative scatterplot conditions. Mean ratings are also shown as points.
#| fig-scap: Dot plots showing individual pre- and post-plot viewing ratings of strength of relatedness for standard and alternative scatterplot conditions.
...
#| fig-env: "figure*"
#| fig-asp: 0.4
#| fig-cap: Histograms illustrating the magnitudes of the difference between pre- and post-plot viewing ratings of strength of relatedness for standard and alternative scatterplots. Difference is calculated by subtracting the ratings made before viewing plots from the ratings made after. Median values are plotted as points.
#| fig-scap: Histograms illustrating the magnitudes of the difference between pre- and post-plot viewing ratings of strength of relatedness in Experiment 5.
...
#| fig-scap: Differences in beliefs about strength of relatedness plotted separately for graph literacy scores, defensive confidence scores, and statement emotional valence.
...
@fig-add-analyses-plots. Participants with higher scores on a graph
literacy test experienced smaller changes in their ratings of strength of
relatedness when viewing standard scatterplots. This is in line with previous work suggesting that those with
...
An opposing pattern of results is seen when participants viewed alternative
scatterplots designed to elicit higher levels of belief change; difference
in belief is mostly stable across different levels of graph literacy. A potential
explanations for this phenomena include the presumably unfamiliar nature of the stimuli in
the alternative scatterplot condition causing participants to attend to the 
stimuli differently, however further work would be required to explore
this in more detail.

Up to a point, all participants with higher scores on the defensive confidence test experienced
...
scatterplot type, defensive confidence, and belief change diverges past \sim 36/60
...
number of directions (see Section \ref{future-work-chap8}, \chap{chap:conclusion},
page \pageref{future-work-chap8}).
...
accomplish. Similarly, the size and opacity decay parameters utilised in the
alternative scatterplot condition have resulted in a large amount of point overlap
as *r* approaches 1, meaning individual point discriminability suffered 
(see @fig-exp5-examples-chap7 on page \pageref{fig-exp5-examples-chap7}).
Further tuning of these decay functions, with
potential adjustment for overlap, would be required if plots such as these were
to be used in real-life visualisations.
