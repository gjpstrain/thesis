...
opacity and size changes can have powerful effects on participants' estimates of
...
\cite{karduni_2020, markant_2023}; this may depend on factors such as prior
...
statements are reproduced in Appendix X2.
...
[^7]: The pre-registration for this hypothesis refers to "emotionality", as did
    an earlier draft of this paper. In response to reviewer comments, I clarify
...
survey items (see Appendix A), along with the six attention check items, in a
...
`r printp(belief_kappa$p.value, add_equals = TRUE)`).
...
Statements with average emotionality ratings between 3 and 5 are statements
...
\cite{xiong_2023, markant_2023}, at this point the decision was made to test
only the statement corresponding to weak beliefs about the strength of
...
valence and strength of correlation scales are low ($\kappa$ =
...
\cite{karduni_2020, markant_2023}, and that scatterplots employing point opacity
...
changed \cite{albarracin_2004}, participant's defensive confidence was measured
using Albarracín and Mitchell's \cite{albarracin_2004} 12-item scale. This scale
is replicated from previous work in Appendix B, and has been utilised more
...
here due to the higher predicted cognitive load of the current tasks, as there
...
viewed standard scatterplots, or group B, in which they viewed alternative
scatterplots designed deliberately to elicit higher levels of belief change.
...
the form of a typical "standard scatterplot" trial from Experiment 1.
...
and strength of relatedness. Interspersed among the experimental items were four
attention check trials which explicitly asked participants to set the slider to
0 or 1.
...
previous chapters, the **buildmer** (version 2.11 \cite{buildmer}) package is
...
novel. It would appear that despite following the normal pattern of results for
...
confidence increased past \~ 36/60. It may be that the unfamiliar nature of the
alternative scatterplots was protective against an unexpected behaviour whereby
very high levels of defensive confidence decrease susceptibility to belief
change.
...
statement emotionality as they pertain to belief change after scatterplot
viewing.
...
in scatterplots. Karduni et al. \cite{karduni_2020} found in 2020 that
...
number of directions, which is discussed in Section \ref{future-work-e5}.
...
contributions of each factor to the effects we have seen here.
