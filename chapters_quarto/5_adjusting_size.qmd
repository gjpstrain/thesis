...
\ref{fig-estimates-by-r-e2} on page \pageref{fig-estimates-by-r-e2} illustrates how participants' mean errors in *r*
...
size of Cohen's *d* = 0.23 (see Table \ref{tbl-efs-e2}(a) on page \pageref{tbl-efs-e2}).
Recent evidence suggests that with regards to
...
#| fig-scap: An example of a bubble chart.
#| fig-asp: 0.6
...
\ref{fig-overplotting-examples} on page \pageref{fig-overplotting-examples} demonstrates
...
-   H~1~: The non-linear decay function, in which scatterplot points become
...
-   H~2~: The use of an inverted non-linear decay function, in which scatterplot
...
as outlined in Section \ref{creating-stimuli}, \chap{chap:gen_methods}, 
page \pageref{creating-stimuli}, and was
...
\ref{results-e3} on page \pageref{results-e3} includes analysis that takes into account the physical
...
from scatterplots. Therefore, point visibility testing was included in this
experiment prior to the experimental items. Participants were shown six
...
not answer at all for a particular question, which is suggestive of a misclick
...
participant saw and responded to each of the 4 (factor levels) $\times$ 45 *r*
values = 180 scatterplots in a fully randomised order. There were four scatterplots
for each of the 45 *r* values corresponding to the four levels of the size decay
condition, examples of which are shown in @fig-exp3-examples-chap5 on page
...
#| include: true
#| fig-cap: Estimated marginal means of error for the four conditions tested in Experiment 3. 95\% confidence intervals are shown. The vertical dashed line represents no estimation error. The overestimation zone is included to facilitate comparison to later work.
#| fig-scap: Estimated marginal means of error for the four conditions tested in Experiment 3.
#| fig-asp: 0.4
...
#| fig-scap: Participants' mean errors in correlation estimates grouped by condition and by \textit{r} value in Experiment 3.
...
size decay condition provided the most promising correction to date; the mean
estimation error is approaching a flat line (see the left-most plot in
@fig-estimates-by-r-e3). The change in the shape of the correlation estimation curve
...
in this experiment was .02, while the error associated with the equivalent opacity decay condition
described in Experiment 2 (see Table \ref{tbl-efs-e2} on page \pageref{tbl-efs-e2}) was .09. This
...
(0.61 vs. 0.19), provides evidence that point size is a stronger encoding
...
attention bias was responsible \cite{sun_2016, hong_2022}. The current
...
standard size condition. While the inverted opacity manipulation used in
\chap{chap:adjusting_opacity} may be employed to correct for the
...
curious given that the standard size condition here is identical to the
