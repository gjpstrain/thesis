...
#| include: true
#| fig-cap: An example of a bubble chart. This plot compares car engine displacement with fuel efficiency (in miles per gallon). Additionally, the number of cylinders in the cars engine are encoded with point size. We can see from this plot that vehicles with higher engine displacement and lower fuel efficiency tend to have a higher number of engine cylinders.
#| fig-asp: 0.6
...
\ref{fig-overplotting-examples} in \chap{chap:adjusting_opacity} demonstrates this
...
in what is termed the *weighted average illusion*. This finding motivated the
point opacity investigations in \chap{chap:adjusting_opacity}. Outside of scatterplots
...
the wider literature described above, two hypotheses are made:
  ...
  smaller as they become further from the regression line, will result in lower
  mean errors in correlation estimation than for linear decay and normal size conditions.
