...
affect people's interpretations of the correlations displayed therein. In a fully-
reproducible, large-sample (N = 150) study, I show that combining point opacity
and size manipulations can produce more powerful effects on correlation estimation
than either effect in isolation. Additionally, I comment on the impact that the adjustment
...
and in Sections \ref{related-work-adjusting-opacity} and \ref{related-work-adjusting-opacity}.
...
\cite{rensink_2012, rensink_2014}, while the work I completed in \chap{adjusting_opacity}
...
lower opacity, is associated with lower salience \cite{healey_2011}, can bias
judgements of mean point position \cite{hong_2021}, increases error in positional
...
but higher levels of salience \cite{healey_2011}, results which are supported by evidence 
...
found that perceptions are biased towards areas that contain larger points \cite{hong_2021}.
...
outline in Section \ref{creating-stimuli}, \chap{chap:gen_methods}. Again, equation
...
concerns. Participants were shown size scatterplots containing between 2 and 7
points; these points were the same size and opacity and the smallest and most
...
A full repeated-measures, 2 $\times$ 2 factorial design was employed. Each participant
...
followed by the screen scale and point visibility tasks described in Section \ref{methods-e4}.
...
order while being asked to use a slider (see Figure \ref{fig-slider}, \chap{chap:general_methods})
...
the work that might be done to achieve this. The combination of inverted inverted
...
change in *r* estimation prevision (see Section \ref{results-e3}, \chap{chap:adjusting_size}).
When opacity was changed in the ways, only an inverted decay does not exhibit
the conventional increase in *r* estimation precision with increase *r* value.
...
both here and in previous work \cite{hong_2021}, that size is a more potent
...
do, however, confirm that there is the scope to bias *r* to greater degrees
...
on correlation estimates, new curves can be derived compare estimates made
...
present experiment are supportive of this notion, with the higher and lowest 
mean estimates being observed in the congruent typical and congruent inverted
orientation conditions. These findings also support dot density \cite{yang_2023}
...
(weights) of thees visual features with regards to perception change as a function
