...
the 1970s, studies asking participants to simply provide a numerical estimate
...
particularly when 0.2 < *r* < 0.6. The nature of this underestimation has been
visualised in @fig-underestimation-curve using a function that relates objective
*r* values to those provided by participants \cite{rensink_2017}.

```{r}
#| label: fig-underestimation-curve
#| include: true
#| fig-cap: Using an equation supplied in Rensink (2017) \cite{rensink_2017}, the nature of correlation underestimation may be visualised. The solid curved line represents underestimation behaviour in that study, while the dashed diagonal line represents theoretically perfect estimation.
#| fig-asp: 1
...
as_tibble(my_rs) %>%
  ...
  ggplot() +
  geom_smooth(aes(x = my_rs, y = under_est), colour = "black", size = 0.5) +
  ylim(0,1) +
  xlim(0,1) +
