...
PCP_plot <- data_with_resid %>% 
  mutate(ID = seq_len(nrow(data_with_resid))) %>%
...
stacked_area_plot <- data_with_resid %>%
  mutate(ID = seq_len(nrow(data_with_resid))) %>%
...
ggarrange(PCP_plot, stacked_area_plot, radial_stack, radar_plot,
          ncol = 2)
```
...
at 100, 400, or 1600 miliseconds. This study found that performance was almost
identical for scatterplots presented for either 400 or 1600 miliseconds, and that
there was only a small deterioration in performance for those presented for 
100 miliseconds. The lack of performance improvement for longer presentation times
both facilitates the rapid collection of large amounts of data and speaks to
the intuituve, perceptual nature of correlation perception in scatterplots.
