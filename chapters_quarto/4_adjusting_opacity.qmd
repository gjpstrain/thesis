...
#| include: true
#| tbl-cap: Estimated Marginal Means (EMMs) of correlation estimation error for plot size and the presence of the opacity decay function.
...
#| include: true
#| tbl-cap: Contrasts between levels of the size factor and opacity decay factor.
...
the paper that forms \label{chap:interactions_opacity_size}, the term "contrast"
...
methodologies (see section in related work about ways of testing correlation perception).
...
#| include: true
#| fig-cap: Examples of the stimuli used in Experiment 1, demonstrated with an \textit{r} value of 0.8. Here, "opacity" refers to the alpha value used by ggplot.
#| fig-asp: 0.275
...
#| label: tbl-efs-e1
#| include: true
#| tbl-cap: Cohen's \textit{d} effect sizes and summary statistics for levels of the opacity factor in Experiment 1. Each effect size is compared to the reference level, full contrast (alpha = 1).
...
#| include: true
#| fig-cap: Participants' mean errors in correlation estimates grouped by factor and by \textit{r} value. The dashed horizontal line represents perfect estimation. Participants were most accurate when presented with the plots featuring higher global point opacity. Error bars show standard deviations of estimates.
...
seen in @tbl-efs-e1 along with means and standard deviations. The
...
correlation change with the objective *r* value in the plot. Points represent
...
perfect estimation. As reported in previous literature (see Sec in related work),
participants underestimated *r* in nearly all cases (revise).
...
(see the non-linear decay curve in @fig-opposing-curve) to relate point opacity
to residuals.
...
Equation 4.1 was used to non-linearly map residuals to ggplot alpha values.
...
#| include: true
#| fig-cap: Examples of the stimuli used in Experiment 2, demonstrated with an \textit{r} value of 0.8. Here, "opacity" refers to the alpha value used by ggplot.
#| fig-asp: 0.275
...
#| label: tbl-efs-e2
#| include: true
#| tbl-cap: Cohen's \textit{d} effect sizes and summary statistics for the opacity decay function factor in Experiment 2. Each effect size is compared to the reference level, full contrast (alpha = 1).
...
gathered from being practically useful. The methods used to not allow for investigation
...
for this overestimation. The non-linear opacity decay function demonstrated
...
using perpendicular and vertical residual distance as bases for setting for opacity
