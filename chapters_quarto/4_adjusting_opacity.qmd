...
designing and analysing the experiments described here, however see Section \ref{ink} for
plots visualising the impacts of point overlap throughout this thesis. Due to this, the
...
using and inverted form of equation 4.1. Examples of the stimuli used in experiment 2 can be seen in
...
Section \ref{related-work-adjusting-opacity} \cite{wehrhahn_1990, champion_2017},
...
magnitudes and opacity values in each condition using *r* values of 0.2, 0.6, and 0.99.
...
plot_decay_slopes <- function(r_values,
  ...
  all_data <- map_dfr(r_values, function(my_desired_r) {
    ...
    # Join hjust values per r_value + decay combination
  ...
  all_data %>%
    ...
    geom_textline(aes(
      colour = decay,
      label  = decay,
      hjust  = label_hjust,       # now a per-row column
      vjust  = -0.3,
    ), size = 2,
    linewidth = 0.25) +
    scale_x_continuous(
...
plot_decay_slopes(
  ...
  hjust_inverted = c(0.8,  0.875,  1.31),
