...
efs <- lme.dscore(e1_model, E1_uniform_adjustments_tidy, type = "lme4") %>%
  ...
  add_row(Effect = "opacityD", d = "", .before = 1) %>%
...
kbl(efs, booktabs = TRUE, digits = 2)
...
seen in @tbl-efs-e1. The largest effect size observed (*d* ~ 0.17) is between the 
...
For each of the 45 *r* values in experiment 2, there were four versions of each plot corresponding to the three levels of point opacity decay function and 
the baseline global full opacity condition. Examples 
of each of these can be seen in@fig-exp2-examples-chap4, demonstrated 
with an *r* value of 0.6.
...
contrasts_extract(e2_model, "opacity") %>%
  mutate(Contrast = str_replace_all(Contrast, labels_e2)) %>%
  separate(Contrast, c(" ", "  "), sep = "-") %>%
  kbl(booktabs = TRUE, digits = c(0,2,3), escape = FALSE) %>%
  add_header_above(c("Contrasts" = 2, "Statistics" = 2))
