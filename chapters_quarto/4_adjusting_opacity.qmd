...
Participants viewed 180 experimental plots in a 3$x$2 factorial design. The first
independent variable, plot size, had three levels, 63%, 100%, and 252% scales. The second
was the simple presence or absence of the opacity decay function (see @fig-pilot-study-examples).
...
contrasts(pilot_data$size) = contr.sum(levels(only_those_passed$size))
contrasts(pilot_data$present) = contr.sum(levels(only_those_passed$present))
...
effects modelling was built whereby participants' errors in correlation estimation
...
to explore estimated marginal means separately for each factor.
