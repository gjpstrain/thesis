...
experimental items, corresponding to \~27,000 individual judgements per 
...
altering stimulus opacity on perception described in Section \ref{related-work-adjusting-opacity}
...
errors (b). The largest effect size observed (*d* \~ 0.16) is between the
...
#| include: true
#| fig-cap: Using \textit{r} values of 0.2, 0.6, and 0.99 to demonstrate the relationship between the size of a point's residual and the alpha value (opacity) rendered. Note that 
#| fig-asp: 0.45
...
plot_decay_slopes <- function(r_values,
  ...
  # Build per-facet hjust lookup: r_label -> decay -> hjust
  ...
  all_data %>%
    ...
    facet_wrap(~ r_value) +
    annotate("text", x = 0.4, y = 1, size = 2, label = "Fully Opaque") +
    annotate("text", x = 0.4, y = 0, size = 2, label = "Fully Transparent")
}

# Each hjust argument is now a vector with one value per r
plot_decay_slopes(
  r_values       = c(0.2,  0.6,  0.9),
  hjust_inverted = c(0.8,  0.8,  1.31),
