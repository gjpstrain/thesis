...
# due to the missing age value, manually add 1 to male gender for 
# experiment 1

E1_uniform_adjustments_tidy_gender <- E1_uniform_adjustments_tidy_gender %>%
  select(-c("NA")) %>%
  mutate(M = M + 1)
```
...
screen size. This was prompted by recent research demonstrating consistent perceptual biases in
...
and the increasing adoption of wearable devices, which by their nature result
in users viewing visualisations of a range of sizes \cite{shandhi_2024}. The pilot
...
implementation of this condition was immature. Throughout this thesis, **decay**
refers to the way in which the opacity or size of a point is changed according
to its distance from the regression line; a **decay function** refers, therefore,
to the mathematical relationship between a point's opacity or size and its residual
magnitude. 

In Experiment 2 onwards, if a
...
in plots of the type seen on the left in @fig-pilot-study-examples.

```{r}
#| label: fig-pilot-study-examples
#| include: true
#| fig-cap: Examples of the experimental stimuli used in the pilot study (opacity decay factor). On the left, the opacity decay function is visible. Note the linear scaling and full opacity range used.
#| fig-scap: Examples of the experimental stimuli used in the pilot study.
...
difficult to comment on the effects of changing the levels of opacity in various
parts of a plot on correlation estimation behaviour. This was later addressed by
...
detailed in Section \ref{recruitment} on page \pageref{recruitment} were implemented.

In the pilot experiment, participants viewed 180 experimental plots in a $3\times2$ factorial design. The first
...
#| include: true
#| tbl-cap: Estimated Marginal Means (EMMs) of correlation estimation error for plot size (a) and the presence of the opacity decay function (b) in the pilot study.
...
#| include: true
#| tbl-cap: Contrasts between levels of the size factor (a) and opacity decay factor (b) in the pilot study.
...
Despite the statistical significance of this finding, I elected at this point
...
systematically to correct for the historical underestimation bias.
...
point opacity as a function of the magnitude of a point's residual is able to bias
estimates of positive correlation upwards to partially correct for a historical
...
#| fig-scap: Demonstrating overplotting as it relates to point opacity.
...
designing and analysing the experiments described here, however see Section \ref{ink} on
page \pageref{ink} for plots visualising the impacts of point overlap throughout this thesis. Due to this, the
...
#| fig-scap: Demonstrating the relationship between `ggplot2` alpha values and point opacity.
...
\chap{chap:related_work}, page \pageref{scatterplots-corr-viz}).
...
estimates of mean point position were biased towards areas of larger or
...
investigated. The second experiment examines how changing point opacity as a function
of distance from the regression line affects perceived correlation. To pre-empt
...
\ref{graph-literacy-related-work} of \chap{chap:related_work},
page \pageref{graph-literacy-related-work}. Early piloting
...
training is provided in Section \ref{training-adjusting-opacity} on
page \pageref{training-adjusting-opacity}. Participants
...
#| fig-scap: Training slide shown to participants.
...
participants saw all  4 (factor levels) $\times$ 45 (*r* values) = 180 
experimental items, corresponding to \sim 27,000 individual judgements per 
experiment, in a fully randomised order.

## Materials {#shared-materials-chap4}

Both experiments used the same 45-*r* value dataset. This dataset was generated
using a uniform distribution for 0.2 < *r* < 0.99. Each experiment featured
a single factor. In Experiment 1, this was the alpha value applied to each 
individual point, and had four levels; alpha = 0.25, 0.5, 0.75, and 1 (full opacity).
Examples of stimuli in Experiment 1 can be seen in @fig-exp1-examples-chap4. In 
Experiment 2, the factor was the implementation of the point opacity decay function,
and had four levels; full opacity, linear decay, in which point opacity is
decreased linearly with residual magnitude, non-linear decay, in which point
opacity is decreased with residual magnitude using equation 4.1, and inverted
non-linear decay, in which point opacity is increased with residual magnitude
using an inverted form of equation 4.1. Examples of the stimuli used in Experiment 2 can be seen in
@fig-exp2-examples-chap4.
...
written in. Experiments 1[^2] and 2[^3] are hosted on Pavlovia.org, while the
Open Science Framework hosts pre-registrations[^4]. There were no deviations 
from the pre-registrations. It is important to note at
...
pilot described above provided evidence against this conclusion, so before
...
estimation interact more generally was important. Given the lack of previous
evidence that correlation perception might be influenced by point opacity,
I could not hypothesise whether or not this might be the case. However, owing to 
the robust effects of altering stimulus opacity on perception described in 
Section \ref{related-work-adjusting-opacity}, page \pageref{related-work-adjusting-opacity}
\cite{wehrhahn_1990, champion_2017}, it was hypothesised that:

-   H~1~: A greater spread of estimates of correlation, measured by standard 
...
(`r printnum(E1_uniform_adjustments_tidy_gender$M, digits = 0)` male,
...
mean age was `r printnum(E1_uniform_adjustments_tidy_age$mean, digits = 1)` (SD =
`r printnum(E1_uniform_adjustments_tidy_age$sd, digits = 1)`). Mean graph literacy score was
`r printnum(E1_uniform_adjustments_tidy_graph_literacy$mean, digits = 1)` (SD =
`r printnum(E1_uniform_adjustments_tidy_graph_literacy$sd, digits = 1)`) out of 30. The mean time
...
be seen in @fig-exp1-examples-chap4, demonstrated with *r* values of 0.2, 0.6, and 
0.99.
...
#| fig-scap: Examples of the stimuli used in Experiment 1.
#| fig-asp: 0.8
...
While I did not make hypotheses based on estimation accuracy, it is the primary 
focus of the statistical analysis due to its relevance to later work.
To investigate the effects of opacity condition on participants' estimates of
correlation, a linear mixed effects model was built with opacity condition as
...
participants. A likelihood ratio test revealed that the model including global point 
...
#| include: true
#| fig-cap: Estimated marginal means of error for the four conditions tested in Experiment 1. 95\% confidence intervals are shown. The vertical dashed line represents no estimation error. The overestimation zone is included to facilitate comparison to later work.
#| fig-scap: Estimated marginal means of error for the four conditions tested in Experiment 1.
#| fig-asp: 0.4
...
observed results could be explained by differences in participants' levels of
...
#| label: tbl-efs-e1
#| include: true
#| tbl-cap: Cohen's \textit{d} effect sizes (a) and summary statistics (b) for levels of the opacity factor in Experiment 1. Each effect size is compared to the reference level, full opacity (alpha = 1).
#| tbl-subcap: ["Effect sizes for opacity factor.", "Means and standard errors of estimation error."]
#| layout-ncol: 2
...
#| fig-scap: Participants' mean errors in correlation estimates grouped by condition and by \textit{r} value.
...
opacity, alpha = 1.0) and each other level of the opacity factor. These
statistics can be seen in Table \ref{tbl-efs-e1}(a) along with means and standard
errors (b). The largest effect size observed (*d* \sim 0.16) is between the
...
line shows mean errors in correlation estimation, and standard deviations of
...
page \pageref{underestimation-related-work}), participants underestimated *r* in nearly all cases.
...
scatterplot points was lower compared to when it was higher, however accuracy
is fairly consistent across experimental conditions. This held true up
...
it pertains to the opacities of scatterplot points, and reports no difference in
...
also played a role. Given the small effect size (Cohen's *d* = 0.16), the
small sample (*N* = 12) in Rensink (2014) \cite{rensink_2014} may have caused the
...
arrangement of point opacity. This experiment builds on the immature opacity
decay employed in the unpublished pilot study (see Section \ref{pilot-study}).
In that study, point opacity was decreased as a function of residual magnitude, but
the range of opacity values used for each plot was the same. In the present experiment,
residual magnitude is hardcoded to point opacity, meaning that if a point has a 
certain residual magnitude value, it will always have the same opacity value.
Additionally, in this experiment, I introduce the concept of different types 
of decay. Linear decay refers to conditions with a linear relationship between
residual magnitude and opacity. Non-linear decay conditions use equation 4.1 to
map residual magnitude to opacity values. Inverted decay conditions feature
scatterplots whose points increase in opacity as a function of residual magnitude
using an inverted form of equation 4.1. Due to alpha values being hard coded
to residual magnitudes, the full range of opacities
is not always accessed depending on the *r* value and the particular condition.
To illustrate this, Figure \ref{fig-opposing-curve} demonstrates the relationships between residual
magnitudes and opacity values in each condition using *r* values of 0.2, 0.6, and 0.9.

```{r}
#| label: fig-opposing-curve
#| include: true
#| fig-cap: Using \textit{r} values of 0.2, 0.6, and 0.9 to demonstrate the relationship between the size of a point's residual and the alpha value (opacity) rendered. Note that as the \textit{r} value increases, the range of alpha values that can be applied to points decreases.
#| fig-scap: Demonstrating the relationship between the size of a point's residual and the alpha value (opacity) rendered.
#| fig-asp: 0.45

plot_decay_slopes <- function(r_values,
                              hjust_inverted, hjust_linear, hjust_non_linear) {
  ...
  r_labels <- paste0("*r* = ", r_values)
  hjust_map <- setNames(
    map(seq_along(r_values), function(i) {
      c(
        "Inverted Decay"   = hjust_inverted[[i]],
        "Linear Decay"     = hjust_linear[[i]],
        "Non Linear Decay" = hjust_non_linear[[i]]
      )
    }),
    r_labels
  )
  
  all_data <- map_dfr(r_values, function(my_desired_r) {
    ...
    data_with_resid %>%
      mutate(
        linear_decay     = ((-my_residuals / 2.6) + 1),
        inverted_decay   = 1 - (0.25)^my_residuals,
        non_linear_decay = (1 + (0.25)^my_residuals) - 1,
        r_value          = paste0("*r* = ", my_desired_r)
      ) %>%
      pivot_longer(
        cols      = ends_with("ay"),
        names_to  = "decay",
      ...
      mutate(
        decay = str_replace_all(decay, "_", " "),
        decay = str_to_title(decay)
      )
  }) %>%
    mutate(label_hjust = map2_dbl(r_value, decay, ~ hjust_map[[.x]][[.y]]))
  
  all_data %>%
    group_by(decay, r_value) %>%
    ...
    facet_wrap(~ r_value) +
    annotate("text", x = 0.75, y = 1, size = 2, label = "Fully Opaque") +
    annotate("text", x = 0.75, y = 0, size = 2, label = "Fully Transparent")
}

plot_decay_slopes(
  r_values       = c(0.2,  0.6,  0.9),
  hjust_inverted = c(0.9,  0.875,  1.31),
  hjust_linear   = c(0.7,  0.7,  1.275),
  hjust_non_linear = c(0.5, 0.7, 1)
)

```

Given the previously reported effects of changing
scatterplot point opacity on other perceptual metrics (see Section
\ref{point-opacity-chap4} on page \pageref{point-opacity-chap4}), and with the 
findings from Experiment 1 in mind, it was hypothesised that:

-   H~1~: the non-linear decay parameter in which point opacity falls with
...
-   H~2~: the use of the inverted non-linear decay parameter, in which point
...
mean age was `r printnum(E2_spatially_dependent_tidy_age$mean, digits = 1)` (SD =
`r printnum(E2_spatially_dependent_tidy_age$sd, digits = 1)`). Mean graph literacy score was
`r printnum(E2_spatially_dependent_tidy_graph_literacy$mean, digits = 1)` (SD =
`r printnum(E2_spatially_dependent_tidy_graph_literacy$sd, digits = 1)`) out of 30. The mean time
taken to complete the experiment was 33 minutes (SD = 10 minutes).
...
@fig-exp2-examples-chap4, demonstrated with *r* values of 0.2, 0.6, and 0.99. Given the shape
of the underestimation curve found in previous work (see Figure
\ref{fig-underestimation-curves}, \chap{chap:related_work},
page \pageref{fig-underestimation-curves}), intuition suggested
employing a symmetrically opposing curve (see the non-linear decay curve in
@fig-opposing-curve) to relate point opacity to residuals. A minimum alpha value of 0.025
was used to ensure that all points were visibly rendered, however, unlike in later
chapters, visibility testing was not carried out.
...
#| fig-scap: Examples of the stimuli used in Experiment 2.
#| fig-asp: 0.8
...
#| include: true
#| fig-cap: Estimated marginal means of error for the four conditions tested in Experiment 2. 95\% confidence intervals are shown. The vertical dashed line represents no estimation error. The overestimation zone is included to facilitate comparison to later work.
#| fig-scap: Estimated marginal means of error for the four conditions tested in Experiment 2.
#| fig-asp: 0.4
...
= `r printp(e2_lit_model.Chisq)`, *p* `r printp(e2_lit_model.p, add_equals = TRUE)`).

```{r}
#| label: tbl-efs-e2
#| include: true
#| tbl-cap: Cohen's \textit{d} effect sizes (a) and summary statistics (b) for the opacity decay function condition in Experiment 2. Each effect size is compared to the reference level, full opacity (alpha = 1).
#| tbl-subcap: ["Effect sizes for opacity decay factor.", "Means and standard errors of estimation error."]
#| layout-ncol: 2
...
observed (\sim 0.23 between full opacity and inverted decay conditions) is small to
moderate, and the effect size between the baseline and the non-linear decay
condition (\sim 0.19) is small. @fig-estimates-by-r-e2 illustrates the effects of
...
#| include: true
#| fig-cap: Participants' mean errors in correlation estimates grouped by condition and by \textit{r} value. The dashed horizontal line represents perfect estimation. Participants were most accurate when presented with the plots featuring the non-linear opacity decay function (far left). Error bars show standard deviations of estimates.
#| fig-scap: Participants' mean errors in correlation estimates grouped by condition and by \textit{r} value in Experiment 2.
...
correlation estimation were lowest when the non-linear opacity decay function was used,
...
observed that as in Experiment 1 (see @fig-estimates-by-r-e1 on page \pageref{fig-estimates-by-r-e1}), and aside from
...
plot_example_function(slopes_0.6 ,"", 1, 0.2, 7) +
...
people are more easily able to interpret and understand, I used a large,
...
possible. Of course, when scatterplots are designed with more than simple
correlation estimation in mind, the techniques described here might not be suitable,
and may impede the completion of other scatterplot tasks. For example, if viewers
were also expected to be able to find outliers, the reduced opacity of exterior
points may hamper their ability to do so.
...
@fig-training-slide-adjusting-opacity on page \pageref{fig-training-slide-adjusting-opacity})
...
estimates of correlation, at least when plots like these are presented sequentially with
...
on the conclusions that can be drawn, although these limitations do not QFprevent
...
The most exciting avenue for related future work is expanding the techniques
developed here to other visual features in isolation and combination.
\chap{chap:adjusting_size} investigates the use of point size in place of
opacity with the techniques described, while \chap{chap:interactions_opacity_size} 
combines the two to explore how these different modalities work together with regards to
the estimation of positive correlation.

# Conclusion {#conclusion-chap4}

In a pair of experiments, I varied the opacity of points in scatterplots both
...
scatterplot can affect participants' performance on a correlation estimation
